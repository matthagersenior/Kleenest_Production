...
Deno.serve(async(req)=>{
  ...
  let pending:Array<{id:string;token:string;platform:string}>=[];let notificationId:string|null=null;
  try{
    ...
    let skipped=0;
    for(const token of tokens){const {data:existing}=await supabase.from('notification_native_push_deliveries').select('id,status,attempts').eq('notification_id',notification.id).eq('token_id',token.id).maybeSingle();if(['submitted','delivered','expired'].includes(existing?.status)){skipped++;continue;}const attempts=Number(existing?.attempts||0);if(attempts>=MAX_DELIVERY_ATTEMPTS){skipped++;continue;}await supabase.from('notification_native_push_deliveries').upsert({notification_id:notification.id,token_id:token.id,status:'pending',attempts:attempts+1,updated_at:new Date().toISOString()},{onConflict:'notification_id,token_id'});pending.push(token);}
  ...
  }catch(error){const message=error instanceof Error?error.message:String(error);if(notificationId&&pending.length){const now=new Date().toISOString();await Promise.all(pending.map(token=>supabase.from('notification_native_push_deliveries').update({status:'failed',last_error:message.slice(0,1000),updated_at:now}).eq('notification_id',notificationId).eq('token_id',token.id)));}return json({error:message},500)}
});

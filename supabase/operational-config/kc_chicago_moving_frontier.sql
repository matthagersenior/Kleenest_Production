-- Canonical operational configuration for the live KC-to-Chicago moving-frontier ingestion strategy.
-- This file mirrors runtime rows that are intentionally mutable by ingestion; it is not a schema migration.
...
set name=excluded.name,state_code=excluded.state_code,priority=excluded.priority,bbox=excluded.bbox,updated_at=now();

-- Preserve existing progress while shelving lower-value legacy directions.
...
-- Nearly-complete/productive northern legacy work remains available behind the new frontier anchors.
update public.national_ingestion_markets

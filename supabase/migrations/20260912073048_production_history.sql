...
set sample_enabled = case when release_state in ('retired','disabled') then false else true end,
    pilot_enabled = case when release_state in ('enabled','mixed','play-gated') then true else false end,
    pilot_mode = case
...
create policy capability_offer_promises_client_deny on public.capability_offer_promises for all to anon,authenticated using(false) with check(false);
...
create policy capability_offer_governance_log_client_deny on public.capability_offer_governance_log for all to anon,authenticated using(false) with check(false);
...
on conflict(offer_key) do update set
  label=excluded.label,audience=excluded.audience,description=excluded.description,required_domains=excluded.required_domains,
  sample_profile=excluded.sample_profile,owner_notes=coalesce(public.capability_offer_promises.owner_notes,excluded.owner_notes),updated_at=now();
...
begin
  ...
  with expanded as (
    ...
    select offer_key,max(label) label,max(audience) audience,max(description) description,
           ...
           max(owner_notes) owner_notes,array_agg(required_domain order by required_domain) required_domains,
  ...
  ) order by offer_key),'[]'::jsonb) into v_result from rollup;
  return v_result;
end;
$function$;
...
begin
  if not public.is_platform_owner_session() then raise exception 'platform owner access required' using errcode='42501'; end if;
  if exists(select 1 from jsonb_object_keys(v_patch) k where k not in ('active','sample_enabled','pilot_enabled','pilot_mode','commercial_state','owner_notes','sample_profile')) then raise exception 'unsupported offer governance field'; end if;

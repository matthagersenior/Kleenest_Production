-- QR Studio canonical foundation v1
...
begin
  ...
  -- Normalize legacy Business customization into the versioned envelope.
...
begin
  ...
  if a not in (
    ...
    raise exception 'Unsupported QR action type: %', a;
  end if;
...
begin
  ...
  if p_qr_id is null then
    ...
    insert into public.qr_codes(
      ...
      v_customization,
      coalesce(nullif(trim(v_patch->>'purpose'),''),v_action_type),
      v_action_type,
      v_action_payload,
      coalesce((v_patch->>'single_use')::boolean,false),
    ...
    where id=p_qr_id and business_id=p_business_id
    for update;
  ...
  if old_q.id is null or v_snapshot is distinct from v_old_snapshot then
    ...
    from public.qr_code_versions where qr_code_id=q.id;
...
create or replace function public.qr_studio_save_template(
  ...
  p_design jsonb default '{}'::jsonb,
  p_default_action jsonb default null
)
returns jsonb
language plpgsql
security definer
...
-- Repair legacy Business QR entrypoints by routing them into the canonical versioned path.

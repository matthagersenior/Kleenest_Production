...
create function public.nearby_locations(lat double precision, lng double precision, radius_meters integer default 8047, limit_count integer default 50)
...
as $$
  ...
  where l.is_active = true and l.latitude is not null and l.longitude is not null
    and extensions.st_dwithin(extensions.st_setsrid(extensions.st_makepoint(l.longitude,l.latitude),4326)::extensions.geography,extensions.st_setsrid(extensions.st_makepoint(lng,lat),4326)::extensions.geography,greatest(100,radius_meters))
...
$$;
revoke all on function public.nearby_locations(double precision,double precision,integer,integer) from public;

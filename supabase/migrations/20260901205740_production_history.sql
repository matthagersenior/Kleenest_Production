...
declare
  v jsonb;
  scheduler_status jsonb := jsonb_build_object(
    'jobname','kleenest-national-ingestion',
    'schedule','*/15 * * * *',
    'active',true,
...
begin
  ...
  begin
    ...
    into scheduler_status
    from cron.job j
    where j.jobname='kleenest-national-ingestion'
    limit 1;

    if scheduler_status is null then
      scheduler_status := jsonb_build_object(
        'jobname','kleenest-national-ingestion',
        'schedule','*/15 * * * *',
        'active',false,
        'source','pg_cron_missing'
      );
    end if;
  ...
  select jsonb_build_object(
    ...
    'today_usage',(select coalesce(jsonb_agg(to_jsonb(u)),'[]'::jsonb) from (
      select source_key,
        sum(requests_used)::int requests_used,
        sum(bytes_downloaded)::bigint bytes_downloaded,
        sum(records_seen)::int records_seen,
        sum(records_imported)::int records_imported,
        sum(records_updated)::int records_updated,
        count(*)::int runs,
        count(*) filter(where status='completed')::int completed_runs,
        count(*) filter(where status='failed')::int failed_runs
    ...
    'last_activity_at',(select max(coalesce(nr.finished_at,nr.started_at)) from public.national_ingestion_runs nr),

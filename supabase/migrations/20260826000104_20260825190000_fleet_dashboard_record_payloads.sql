...
begin
  ...
  maint as (
    select
      count(*) filter (where status in ('scheduled','due','overdue')) due,
    ...
    from public.fleet_maintenance_records fm
    where business_id=p_business_id
  ),
  loc as (
    select
    ...
    where is_active=true and business_id=p_business_id
  ),
  opp as (
    select
      count(*) opportunities,
      count(*) filter (where needs_fresh_observation=1) stale_opportunities
    from public.fleet_service_opportunities
    where business_id=p_business_id
  )
  select jsonb_build_object(
    'vehicles', veh.total,
    'vehicles_active', veh.active,
    'odometer_miles', veh.odometer,
    'drivers', drv.total,
    'drivers_active', drv.active,
    'routes', rte.total,
    'routes_active', rte.active,
    'routes_completed', rte.completed,
    'route_miles', rte.miles,
    'route_minutes', rte.minutes,
    'open_alerts', alt.open,
    'alerts_total', alt.total,
    'operational_events', ops.events,
    'safety_events', ops.safety_events,
    'telemetry_miles', ops.miles,
    'telemetry_minutes', ops.minutes,
    'maintenance_due', maint.due,
    'maintenance_spend', maint.maintenance_spend,
    'location_total', loc.total,
    'location_verified', loc.verified,
    'location_accessible', loc.accessible,
    'location_changing_tables', loc.changing_tables,
    'service_opportunities', opp.opportunities,
    'stale_service_opportunities', opp.stale_opportunities,
    'vehicle_records', veh.records,
    'driver_records', drv.records,
    'route_records', rte.records,
    'alert_records', alt.records,
    'maintenance_records', maint.records
  )
  into v

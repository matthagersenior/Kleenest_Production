...
begin
  ...
  set display_name=v_display_name, username=v_username, bio=v_bio
  where id=v_uid
  returning * into v_row;
  if v_row.id is null then raise exception 'Profile not found'; end if;

  return jsonb_build_object('id',v_row.id,'display_name',v_row.display_name,'username',v_row.username,'avatar_url',v_row.avatar_url,'bio',v_row.bio,'points',v_row.points,'level',v_row.level,'streak',v_row.streak,'total_check_ins',v_row.total_check_ins,'total_reviews',v_row.total_reviews);
end;
$$;
...
begin
  ...
  return jsonb_build_object('id',v_row.id,'avatar_url',v_row.avatar_url);
end;
$$;

...
begin
 ...
 if not exists(select 1 from public.profiles where id=auth.uid() and (is_admin=true or lower(coalesce(role::text,'')) in ('admin','owner','platform_admin'))) then raise exception 'admin authorization required'; end if;
 v_table:=lower(p_resource);
 if v_table not in ('profiles','businesses','locations','location_amenities','location_photos','business_members','business_campaigns','business_events','contests','promotions','reviews','social_posts','badges','reports','support_requests','progression_actions','user_badges','progression_games','partner_programs','partner_agreements','contest_entries','app_business_memberships') then raise exception 'admin resource not allowed: %',p_resource; end if;
 if v_table='app_business_memberships' and lower(p_action)<>'list' then raise exception 'business memberships is read-only'; end if;

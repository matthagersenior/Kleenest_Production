...
declare v_program uuid; v_a uuid; v_b uuid; v_l1 uuid; v_l2 uuid; v_result jsonb;
begin
 ...
 if v_program is null then
  insert into public.partner_programs(business_id,name,description,enabled,preferred_access,status,is_demo_test)
  values(v_a,'Demo Preferred Partner Program','Controlled demo benefit-swap program',true,true,'active',true) returning id into v_program;
 else
  update public.partner_programs set enabled=true,preferred_access=true,status='active' where id=v_program;
 end if;
 insert into public.partner_agreements(business_id,partner_business_id,partner_program_id,name,status,is_demo_test)
 select v_a,v_b,v_program,'Demo Partnership','active',true
...
revoke execute on function public.ensure_demo_partner_network() from public,anon,authenticated;

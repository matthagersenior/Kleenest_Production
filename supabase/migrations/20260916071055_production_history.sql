...
begin
  ...
  values
    ...
    (p_owner,'data-integrity','Data integrity','Orphaned or contradictory canonical records.','data_integrity','Data',true,'immediate','warning',true,1440,30),
    (p_owner,'ingestion-storage','Ingestion & storage','Storage guards, scheduler health and failed ingestion runs.','ingestion_storage','Operations',true,'immediate','warning',true,10080,30),
...
set dedupe_window_minutes=case
  when source_key='ingestion_storage' then 10080
  when source_key='data_integrity' then 1440
  else dedupe_window_minutes
end,
updated_at=now()
where source_key in ('ingestion_storage','data_integrity');

update public.owner_email_notification_events e
set status='suppressed',
    last_error='Suppressed because the ingestion scheduler is expected to be inactive while ingestion is storage-paused.',
    updated_at=now()
where e.status='queued'
  and e.source_key='ingestion_storage'
  and e.dedupe_key='scheduler:inactive'
  and exists(select 1 from public.national_ingestion_storage_guard g where g.singleton=true and g.paused);

create or replace function public.owner_email_provider_status()
returns jsonb
language plpgsql
security definer
set search_path=''
...
begin
  ...
  select exists(select 1 from vault.secrets where name='owner_email_resend_api_key') into v_configured;
  select decrypted_secret into v_from from vault.decrypted_secrets where name='owner_email_from_address' limit 1;
  return jsonb_build_object(
    'provider','resend',
    'configured',v_configured,
    'from_address',coalesce(v_from,'Kleenest Owner <onboarding@resend.dev>')
  );
end
$$;

create or replace function public.owner_configure_email_provider(
  p_api_key text,
...
declare
  v_uid uuid:=auth.uid();
  v_id uuid;
  v_from text:=coalesce(nullif(trim(p_from_address),''),'Kleenest Owner <onboarding@resend.dev>');
  v_state jsonb;
begin
  if v_uid is null or not public.is_platform_owner(v_uid) then raise exception 'Platform owner access required'; end if;
  if length(trim(coalesce(p_api_key,'')))<12 then raise exception 'A valid Resend API key is required'; end if;

  select id into v_id from vault.secrets where name='owner_email_resend_api_key' limit 1;
  if v_id is null then
    perform vault.create_secret(trim(p_api_key),'owner_email_resend_api_key','Kleenest Owner operational email provider key');
  else
    perform vault.update_secret(v_id,trim(p_api_key),'owner_email_resend_api_key','Kleenest Owner operational email provider key',null);
  end if;

  select id into v_id from vault.secrets where name='owner_email_from_address' limit 1;
  if v_id is null then
    perform vault.create_secret(v_from,'owner_email_from_address','Kleenest Owner operational email sender');
  else
    perform vault.update_secret(v_id,v_from,'owner_email_from_address','Kleenest Owner operational email sender',null);
  end if;

  v_state:=jsonb_build_object('provider','resend','configured',true,'from_address',v_from);
  insert into public.platform_owner_control_audit(owner_user_id,domain,action,target_key,new_state,reason)
  values(v_uid,'owner_email_notifications','configure_provider','resend',v_state,p_reason);
  return v_state;
end
$$;

create or replace function public.owner_email_worker_provider_config()
returns jsonb
language plpgsql
security definer
set search_path=''
...
begin
  if coalesce(auth.jwt()->>'role','')<>'service_role' then raise exception 'Service role required'; end if;
  select decrypted_secret into v_key from vault.decrypted_secrets where name='owner_email_resend_api_key' limit 1;
  select decrypted_secret into v_from from vault.decrypted_secrets where name='owner_email_from_address' limit 1;
  return jsonb_build_object(
    'api_key',v_key,
    'configured',v_key is not null and length(v_key)>0,
    'from_address',coalesce(v_from,'Kleenest Owner <onboarding@resend.dev>')
  );
end
$$;

revoke all on function public.owner_email_provider_status() from public,anon;
revoke all on function public.owner_configure_email_provider(text,text,text) from public,anon;
grant execute on function public.owner_email_provider_status() to authenticated;
grant execute on function public.owner_configure_email_provider(text,text,text) to authenticated;

revoke all on function public.owner_email_worker_provider_config() from public,anon,authenticated;
grant execute on function public.owner_email_worker_provider_config() to service_role;

...
with cases as (
  ...
  select count(*)::int issue_count,
    ...
    count(distinct amenity_id)::int affected_amenities,
    max(resolved_at) latest_recovery_at
  from cases
)
select jsonb_build_object(
  'location_id',p_location_id,
  'issue_count',issue_count,'resolved_count',resolved_count,'community_confirmed',community_confirmed,
  'failed_fixes',failed_fixes,'escalated',escalated,'active_count',active_count,'affected_amenities',affected_amenities,
  'latest_recovery_at',latest_recovery_at,
  'recovery_confidence_score',greatest(0,least(100,
    case when issue_count=0 then 100 else
      55
      + least(25,community_confirmed*12)
      + least(15,resolved_count*4)
      - least(35,failed_fixes*20)
      - least(20,active_count*10)
      - least(15,escalated*5)
      - least(15,greatest(issue_count-affected_amenities,0)*4)
    end
  ))::int,
  'recovery_state',case
    when issue_count=0 then 'no_recent_issues'
    when failed_fixes>0 then 'recovery_unstable'
    when active_count>0 then 'recovery_in_progress'
    when community_confirmed>0 then 'community_confirmed'
    when resolved_count>0 then 'business_reported_recovery'
...
begin
  ...
  with cases as (
    ...
    select *,
      case when failed_fixes>0 then 'root_cause_inspection'
           when recurrence_count>=3 then 'preventive_maintenance_plan'
           when escalated_count>0 then 'sla_process_review'
           when recurrence_count>=1 then 'increase_inspection_frequency'
           when confirmed_fixes=0 then 'request_community_confirmation'
           else 'monitor' end recommended_action,
      case when failed_fixes>0 or recurrence_count>=3 then 'critical'
           when escalated_count>0 or recurrence_count>=2 then 'high'
           when recurrence_count>=1 or confirmed_fixes=0 then 'watch'
           else 'stable' end prevention_priority,
...
begin
  ...
  with cases as (
    ...
    select l.id location_id,l.name location_name,
      count(c.id)::int issue_count,
      greatest(count(c.id)-count(distinct c.amenity_id) filter(where c.id is not null),0)::int repeat_issue_count,
      count(c.id) filter(where c.status in('open','assigned','in_progress'))::int active_count,
      count(c.id) filter(where coalesce(c.escalation_level,0)>0)::int escalated_count,
      count(c.id) filter(where c.resolution_snapshot->>'community_confirmation_outcome'='still_broken')::int failed_fix_count,
      count(c.id) filter(where c.resolution_snapshot->>'community_confirmation_outcome'='confirmed')::int confirmed_fix_count,
      max(c.opened_at) latest_issue_at
    ...
    select *,
      greatest(0,least(100,100-least(40,failed_fix_count*25)-least(30,repeat_issue_count*10)-least(20,active_count*8)-least(15,escalated_count*5)+least(10,confirmed_fix_count*3)))::int prevention_readiness_score,
      case when failed_fix_count>0 then 'root_cause_inspection'
           when repeat_issue_count>=3 then 'preventive_route'
           when escalated_count>0 then 'sla_review'
           when active_count>0 then 'active_response'
           when repeat_issue_count>0 then 'inspection_watch'
           else 'monitor' end recommended_action
    from grouped
  )
...
begin
  ...
  from public.business_restroom_remediation_cases c
  where c.business_id=new.business_id and c.location_id=new.location_id and c.amenity_id=new.amenity_id and c.id<>new.id and c.opened_at>=now()-interval '90 days';

  insert into public.notifications(user_id,type,title,body,data)
  select bm.user_id,
         case when v_recent>=2 then 'business_remediation_recurrence_escalated' else 'business_remediation_opened' end,

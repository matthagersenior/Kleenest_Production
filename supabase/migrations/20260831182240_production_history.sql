...
select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'business_response_status',case
    when r.status='open' then 'reported'
    when r.status in ('assigned','in_progress') then 'being_addressed'
    when r.status='resolved' and (s.latest_attention_at is null or r.resolved_at>=s.latest_attention_at) then 'addressed'
    else null
  end,
  'business_response_at',case
    when r.status='resolved' and (s.latest_attention_at is null or r.resolved_at>=s.latest_attention_at) then r.resolved_at
    when r.status='in_progress' then coalesce(r.started_at,r.assigned_at,r.opened_at)
    when r.status='assigned' then coalesce(r.assigned_at,r.opened_at)
    when r.status='open' then r.opened_at
    else null
  end,

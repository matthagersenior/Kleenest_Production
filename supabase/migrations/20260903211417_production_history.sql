...
begin
  if not public.business_enterprise_authorized(p_business_id) then
    raise exception 'Enterprise entitlement and business management access required';
  end if;
  ...
  business_rows as (
    ...
    join portfolio_business_ids p on p.business_id=b.id
  ),
  location_rows as (
...
revoke all on function public.enterprise_operational_portfolio_snapshot(uuid) from public;

...
values('trust_mission_visit_bonus','Complete a trust mission with a verified visit',5,true)
...
begin
  ...
  select * into v_mission from public.user_trust_missions where user_id=v_user and status='active' order by started_at desc limit 1 for update;
  if not found then
    select * into v_mission from public.user_trust_missions where user_id=v_user and status='completed' and qualifying_review_id=p_review_id order by completed_at desc limit 1;
    if found then select l.name into v_location_name from public.locations l where l.id=v_mission.location_id; return jsonb_build_object('id',v_mission.id,'location_id',v_mission.location_id,'location_name',v_location_name,'status','completed','priority',v_mission.priority,'goal',v_mission.goal,'reward_points',v_mission.reward_points,'started_at',v_mission.started_at,'completed_at',v_mission.completed_at,'completion_evidence',v_mission.completion_evidence); end if;
  ...
  perform public.record_progression_metric_event('trust_mission_completed','trust_mission',v_mission.id,1,null,jsonb_build_object('idempotency_key','trust-mission:'||v_mission.id::text,'location_id',v_mission.location_id,'review_id',v_review.id,'goal_satisfied',v_goal_satisfied));
  perform * from public.quest_dispatch_event(v_user,'trust_mission_completed',v_mission.location_id,v_checkin.id,null,null,jsonb_build_object('mission_id',v_mission.id,'review_id',v_review.id,'goal_satisfied',v_goal_satisfied));
  update public.user_trust_missions set status='completed',qualifying_review_id=v_review.id,qualifying_check_in_id=v_checkin.id,reward_points=v_points,
    completion_evidence=jsonb_build_object('verified_visit_count',coalesce(v_summary.verified_visit_count,0),'photo_evidence_count',coalesce(v_summary.photo_evidence_count,0),'amenity_evidence_count',coalesce(v_summary.amenity_evidence_count,0),'latest_verified_at',v_summary.latest_verified_at,'review_photo_count',v_photo_count,'review_amenity_count',v_amenity_count,'goal_satisfied',v_goal_satisfied),completed_at=now(),completion_processed_at=now(),updated_at=now()
  ...
  insert into public.social_activity(user_id,actor_user_id,activity_type,location_id,metadata) values(v_user,v_user,'trust_mission_completed',v_mission.location_id,jsonb_build_object('mission_id',v_mission.id,'review_id',v_review.id,'reward_points',v_points,'goal_satisfied',v_goal_satisfied,'summary',case when v_goal_satisfied then 'Completed the full evidence goal' else 'Completed a verified visit; supplemental evidence is still useful' end));
  insert into public.notifications(user_id,type,title,body,data) values(v_user,'progress_trust_mission_completed','Trust mission complete',format('You strengthened %s with a verified contribution and earned %s bonus points%s.',coalesce(v_location_name,'this restroom'),v_points,case when v_goal_satisfied then ' for completing the full evidence goal' else '' end),jsonb_build_object('mission_id',v_mission.id,'location_id',v_mission.location_id,'review_id',v_review.id,'reward_points',v_points,'goal_satisfied',v_goal_satisfied,'route','/play'));

...
as $$
declare
 v_radius integer:=greatest(1000,least(coalesce(p_radius_m,16093),80467)); v_limit integer:=greatest(25,least(coalesce(p_limit,1000),2000));
 v_tier text; v_session uuid; v_locations jsonb; v_lat_delta double precision; v_lng_delta double precision;
 v_category text:=nullif(trim(lower(coalesce(p_category,''))),''); v_search text:=nullif(trim(lower(coalesce(p_search,''))),'');
begin
 ...
 select coalesce(jsonb_agg(to_jsonb(x) order by x.distance_meters),'[]'::jsonb) into v_locations from (
   ...
   greatest(0,least(100,round((0.45*coalesce(bi.confidence,case when l.bathroom_verification_status in('verified','confirmed') then 95 when l.place_type='restroom' then 85 else 0 end)+0.20*least(100,coalesce(bi.evidence_count,0)*10)+0.20*case when l.bathroom_verification_status in('verified','confirmed') then 100 when l.place_type='restroom' then 75 else 0 end+0.15*greatest(0,100-extract(epoch from (now()-coalesce(bi.updated_at,l.updated_at)))/86400*5))::numeric,0)))::integer trust_score,
   greatest(0,least(100,round((100-extract(epoch from (now()-coalesce(bi.updated_at,l.updated_at)))/86400*5)::numeric,0)))::integer freshness_score,

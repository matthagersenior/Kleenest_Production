...
begin
 ...
 for item in select value from jsonb_array_elements(p_rows) loop
  ...
  v_name:=coalesce(nullif(item->>'name',''),'Public Place'); v_place_type:=coalesce(nullif(item->>'place_type',''),'place'); v_address:=nullif(item->>'address',''); v_city:=nullif(item->>'city',''); v_state:=nullif(item->>'state',''); v_postal:=nullif(item->>'postal_code',''); v_phone:=nullif(item->>'phone',''); v_website:=nullif(item->>'website',''); v_source_meta:=coalesce(item->'source_metadata','{}'::jsonb);
...
comment on function public.ingest_external_locations(text,jsonb) is 'Privileged canonical external-location ingestion. Browser roles are denied; authenticated callers must have auth.uid(), while trusted service-role/SECURITY DEFINER execution is permitted for scheduled ingestion.';

...
begin
  if auth.uid() is null then raise exception 'Authentication required for ingestion'; end if;
  ...
  for item in select value from jsonb_array_elements(p_rows) loop
    ...
    values('location',loc_id,loc_id,'external_data_import','maps.external.'||p_source_key,'external_location_records',rec_id,1,p_source_key,jsonb_build_object('external_id',ext_id,'place_type',v_place_type,'verification','external_source'),now(),now(),'valid',0.75,p_source_key||':'||ext_id,'{}'::jsonb) on conflict do nothing;
  end loop;

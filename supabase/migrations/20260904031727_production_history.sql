...
begin
 ...
 if v_scope in ('local','city','state') and (v_lat is null or v_lon is null or v_city is null or v_state is null) then
   select coalesce(v_lat,l.latitude),coalesce(v_lon,l.longitude),coalesce(v_city,lower(l.city)),coalesce(v_state,lower(l.state))
   into v_lat,v_lon,v_city,v_state
   from public.progression_events_v2 e join public.locations l on l.id=e.location_id
   where e.user_id=v_user and l.latitude is not null and l.longitude is not null order by e.created_at desc limit 1;
 end if;
 with eligible as (
   ...
   where e.status='awarded'
     and case v_scope
       ...
       when 'specialty' then a.specialty is not null and (v_specialty is null or lower(a.specialty)=v_specialty)
       when 'campaign' then v_objective is not null and nullif(e.subject->>'objective_id','')=v_objective::text
       when 'contest' then (v_objective is null and e.action='contest_place') or (v_objective is not null and (nullif(e.subject->>'objective_id','')=v_objective::text or e.action='contest_place'))
       else false end
 ), scored as (
 ...
 ), ranked as (select user_id,score,dense_rank() over(order by score desc,user_id) rank from scored where score>0)

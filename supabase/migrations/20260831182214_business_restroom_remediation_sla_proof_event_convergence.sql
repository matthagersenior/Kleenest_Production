...
as $$
  select case
    when coalesce(p_priority,0) >= 90 then 4
    when coalesce(p_priority,0) >= 80 then 8
    when coalesce(p_priority,0) >= 70 then 12
    when coalesce(p_priority,0) >= 60 then 24
    else 48
  end;
$$;
...
create trigger trg_apply_restroom_remediation_sla
before insert or update of priority,status on public.business_restroom_remediation_cases
for each row execute function public.apply_restroom_remediation_sla();

update public.business_restroom_remediation_cases
set due_at = opened_at + make_interval(hours => public.restroom_remediation_sla_hours(priority))
where due_at is null;

alter table public.business_restroom_remediation_cases alter column due_at set not null;
create index if not exists business_restroom_remediation_due_idx
on public.business_restroom_remediation_cases(status,due_at,escalation_level)
...
declare
  ...
  v_reporter uuid;
begin
  select coalesce(l.business_id,(select bl.business_id from public.business_locations bl where bl.location_id=new.location_id limit 1))
  into v_business_id
...
create trigger trg_sync_restroom_remediation_from_observation
after insert or update of status,confidence,metadata on public.location_amenity_observations
for each row execute function public.sync_restroom_remediation_from_observation();
...
begin
  ...
  loop
    ...
    if coalesce(r.escalation_level,0) < v_target then
      update public.business_restroom_remediation_cases
      set escalation_level=v_target,escalated_at=now(),updated_at=now()
      where id=r.id;
...
create or replace function public.business_manage_restroom_remediation(
  p_business_id uuid,
  p_case_id uuid,
  p_action text,
...
begin
  ...
  if p_action='assign' then
    ...
    insert into public.notifications(user_id,type,title,body,data)
    values(p_assigned_to,'business_remediation_assignment','Restroom issue assigned',coalesce((select l.name from public.locations l where l.id=c.location_id),'A restroom')||' needs attention.',jsonb_build_object('business_id',p_business_id,'case_id',c.id,'location_id',c.location_id,'amenity_id',c.amenity_id,'due_at',c.due_at,'priority',c.priority));
    ...
    if p_proof_media_id is not null then
      select p.storage_path into v_proof_path
      from public.location_photos p
      join public.locations l on l.id=p.location_id
      where p.id=p_proof_media_id and p.location_id=c.location_id and l.business_id=p_business_id;
    ...
    values(c.location_id,auth.uid(),c.amenity_id,'present',0.75,'business_remediation',p_proof_media_id,trim(p_notes),now(),jsonb_build_object('source','business_remediation','business_id',p_business_id,'remediation_case_id',c.id,'sentiment','resolved_by_business','proof_media_id',p_proof_media_id,'proof_storage_path',v_proof_path))
    returning id into v_obs;

    update public.business_restroom_remediation_cases
    set status='resolved',resolved_at=now(),assigned_to=coalesce(assigned_to,auth.uid()),assigned_at=coalesce(assigned_at,now()),started_at=coalesce(started_at,now()),
        resolution_notes=trim(p_notes),resolution_observation_id=v_obs,resolution_media_id=p_proof_media_id,
    ...
    if v_reporter is not null and v_reporter<>auth.uid() then
      insert into public.notifications(user_id,type,title,body,data)
      values(v_reporter,'business_remediation_resolved','A restroom issue you reported was addressed',coalesce((select l.name from public.locations l where l.id=c.location_id),'A restroom')||' reported a business remediation update.',jsonb_build_object('business_id',p_business_id,'case_id',c.id,'location_id',c.location_id,'amenity_id',c.amenity_id,'resolution_observation_id',v_obs,'proof_media_id',p_proof_media_id,'proof_available',p_proof_media_id is not null));
    end if;
  else
  ...
  return (
    select to_jsonb(x) from (
      ...
      from public.business_restroom_remediation_cases rc
      join public.locations l on l.id=rc.location_id
      join public.amenities a on a.id=rc.amenity_id
      left join public.profiles p on p.id=rc.assigned_to
      left join public.location_photos lp on lp.id=rc.resolution_media_id
      where rc.id=c.id
    ) x
  );
end;

-- Keep ingestion focused on canonical locations and compact source identity.
-- Transitional address-backfill snapshots and duplicate external import events are not core product data.

-- The canonical ingest function already writes address fields directly to public.locations,
-- so the external-record trigger and its duplicate backfill snapshot are unnecessary.
drop trigger if exists trg_sync_external_location_address on public.external_location_records;

truncate table public.location_address_backfills;

-- Remove machine-generated provenance events that duplicate external_location_records.
-- Preserve genuine product/user events such as search, view, route and check-in activity.
delete from public.data_feature_events
where event_type = 'external_data_import'
  and feature_code like 'maps.external.%';

-- External records only need stable identity/linkage and normalized columns for deduplication.
-- Full raw payloads are already represented in the canonical location source metadata where useful.
update public.external_location_records
set raw_data = '{}'::jsonb
where raw_data <> '{}'::jsonb;

-- Capability audit reports are diagnostic artifacts, not runtime product data. Keep only the five newest.
delete from public.capability_audit_runs
where id not in (
  select id from public.capability_audit_runs order by executed_at desc, id desc limit 5
);
 ...
 for item in select value from jsonb_array_elements(p_rows) loop
  begin
   ...
   if loc_id is null and v_name !~* '^(Public Restroom|Unnamed )' then
    select l.id into loc_id from public.locations l where l.latitude is not null and l.longitude is not null and abs(l.latitude-v_lat)<0.00025 and abs(l.longitude-v_lng)<0.00025 and lower(coalesce(l.name,''))=lower(v_name) limit 1;
   end if;
   ...
   on conflict(source_id,external_id) do update set location_id=excluded.location_id,latitude=excluded.latitude,longitude=excluded.longitude,name=excluded.name,raw_data='{}'::jsonb,last_seen_at=now(),active=true returning id into rec_id;

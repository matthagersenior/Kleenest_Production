...
begin
 ...
 select greatest(coalesce((g.rules->>'max_score')::integer,1000000),0),g.name
 into max_score,game_name
 from public.progression_games g
 where g.code=row.game_code and g.enabled=true;
 if not found then raise exception 'Game is unavailable'; end if;
 ...
 if actor=row.creator_id then
   ...
   set creator_score=safe_score,
       metadata=metadata||coalesce(p_metadata,'{}'::jsonb)||jsonb_build_object('score_model','arena_v3','max_score',max_score)
   where id=row.id;
 else
   if row.invitee_score is not null then raise exception 'Your score is already recorded'; end if;
   update public.game_challenges
   set invitee_score=safe_score,
       metadata=metadata||coalesce(p_metadata,'{}'::jsonb)||jsonb_build_object('score_model','arena_v3','max_score',max_score)
   where id=row.id;
 end if;
 ...
 perform public.record_game_result(
   ...
   safe_score,
   0,
   jsonb_build_object('mode','multiplayer','challenge_id',row.id::text,'submitted_score',raw_score,'score_model','arena_v3')
   ||coalesce(p_metadata,'{}'::jsonb)
 );
 ...
 if row.creator_score is not null and row.invitee_score is not null then
   ...
   set status='completed',
       winner_id=case when creator_score=invitee_score then null when creator_score>invitee_score then creator_id else invitee_id end,
       completed_at=now()
   ...
   values(
     opponent_id,'game_challenge','Bathroom trust match completed',
     coalesce(actor_name,'A Kleenest player')||' finished your '||coalesce(game_name,row.game_code)||' challenge. Open Game Center to see the result.',
     jsonb_build_object('challenge_id',row.id,'game_code',row.game_code,'route','/games')
   );
 end if;
 return row;
end;

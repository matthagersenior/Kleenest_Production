...
begin
 ...
 if v_table not in ('profiles','businesses','locations','location_amenities','business_members','contests','promotions','reviews','social_posts','badges','reports','support_requests','progression_actions','user_badges','business_campaigns','business_events','partner_programs','partner_agreements','progression_games','contest_entries','contest_leaderboards') then raise exception 'admin resource not allowed: %',p_resource; end if;
 ...
 if lower(p_action)='list' then
   v_sql:=format('select coalesce(jsonb_agg(to_jsonb(x)),''[]''::jsonb) from (select * from public.%I limit 100) x',v_table);
 ...
 if lower(p_action)='create' then
   ...
   execute v_sql into v_result using p_payload; return v_result;
 end if;
 select string_agg(format('%I = r.%I',key,key),', ' order by key) into v_select_cols from jsonb_object_keys(p_payload) key where key<>'id';
 v_sql:=format('update public.%I t set %s from jsonb_populate_record(null::public.%I,$1) r where t.id=$2 returning to_jsonb(t.*)',v_table,v_select_cols,v_table);

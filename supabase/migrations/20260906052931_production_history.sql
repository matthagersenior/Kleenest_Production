...
BEGIN
  ...
  RETURN QUERY
  WITH candidates AS (
    SELECT l.*, p.id AS place_id, p.name AS place_name, p.category AS place_category,
      p.rating AS place_rating, p.review_count AS place_review_count, p.is_verified AS place_verified,
    ...
    WHERE l.is_active=true AND l.geom IS NOT NULL
      ...
      AND (cardinality(v_names)=0 OR
        (v_match='any' AND EXISTS (
          SELECT 1 FROM public.location_amenities la JOIN public.amenities a ON a.id=la.amenity_id
          WHERE la.location_id=l.id AND lower(trim(a.name))=ANY(v_names)
        )) OR
        (v_match='all' AND (
          SELECT count(DISTINCT lower(trim(a.name))) FROM public.location_amenities la JOIN public.amenities a ON a.id=la.amenity_id
          WHERE la.location_id=l.id AND lower(trim(a.name))=ANY(v_names)
        )=cardinality(v_names))
      )
  ...
  SELECT jsonb_build_object(
    ...
    'osm_tags',coalesce(c.source_metadata->'osm_tags','{}'::jsonb),'business_id',coalesce(c.claimed_business_id,c.business_id),
    'business_name',NULL,'business_logo_url',NULL,'place_type',c.place_type,'phone',c.phone,'website',c.website,'description',c.description,
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'latitude',c.latitude,'longitude',c.longitude,'distance_to_route_meters',c.route_dist,'route_fraction',c.route_pos,
    'source',c.source,'source_dataset',c.source_dataset,'source_external_id',c.source_external_id,'is_verified',coalesce(c.place_verified,false),
    'rating',coalesce(c.place_rating,c.rating,0),'review_count',coalesce(c.place_review_count,c.review_count,0),'cleanliness_pct',c.cleanliness_pct,'verification_confidence',c.verification_confidence,
    'amenities',coalesce((SELECT jsonb_agg(DISTINCT jsonb_build_object('name',a.name,'category',a.category)) FROM public.location_amenities la JOIN public.amenities a ON a.id=la.amenity_id WHERE la.location_id=c.id),'[]'::jsonb),
    'fixtures',coalesce((SELECT jsonb_build_object('stalls',f.stalls,'urinals',f.urinals,'sinks',f.sinks,'hand_dryers',f.hand_dryers,'changing_tables',f.changing_tables,'showers',f.showers) FROM public.location_fixtures f WHERE f.location_id=c.id LIMIT 1),'{}'::jsonb),

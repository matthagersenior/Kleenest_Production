...
begin
  ...
  return jsonb_build_object(
    ...
    'claims',coalesce((select jsonb_agg(jsonb_build_object('id',c.id,'location_id',c.location_id,'claimed_by',c.claimed_by,'status',c.status,'created_at',c.created_at) order by c.created_at desc) from public.location_claims c where c.business_id=p_business_id),'[]'::jsonb)
  );
end $$;

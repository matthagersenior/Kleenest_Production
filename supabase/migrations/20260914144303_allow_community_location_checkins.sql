-- Allow GPS/geofence check-ins at active community locations.
-- Location/business verification status is separate from visit verification:
-- users must be able to physically verify pending/community locations so their
-- contributions can improve the trust record. Rejected locations remain blocked.
...
begin
  ...
  if v_last_check is not null then
    ...
    if v_last_departure is null then
      raise exception 'LEAVE_REQUIRED_BEFORE_REPEAT_CHECK_IN';
    end if;
  end if;

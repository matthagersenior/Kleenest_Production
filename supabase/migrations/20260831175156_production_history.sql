...
begin
 ...
 select p_business_id,ao.location_id,ao.amenity_id,ao.id,
        least(100, greatest(40,
          55 + case when ao.status='absent' then 20 else 0 end
             + case when coalesce((ao.metadata->>'sentiment'),'')='needs_attention' then 15 else 0 end
             + case when ao.observed_at >= now()-interval '7 days' then 10 else 0 end)),
 ...
 where c.business_id=p_business_id and c.status in ('open','assigned','in_progress')
   and exists (
     ...
     where o.location_id=c.location_id and o.amenity_id=c.amenity_id
       ...
       and o.status='present' and coalesce(o.confidence,0)>=0.7
       and coalesce(o.metadata->>'sentiment','')<>'needs_attention'
   );

...
begin
  ...
  select u.email, coalesce(u.raw_user_meta_data->>'display_name', pg_catalog.split_part(u.email,'@',1))
    into v_email,v_name
...
begin
  ...
  on conflict(id) do update set
    ...
    is_demo_test=public.profiles.is_demo_test or excluded.is_demo_test;
  select pg_catalog.to_jsonb(p) into v_profile from public.profiles p where p.id=v_user;
...
begin
  ...
  values(new.id,coalesce(new.raw_user_meta_data->>'full_name',new.email,'Kleenest member'))

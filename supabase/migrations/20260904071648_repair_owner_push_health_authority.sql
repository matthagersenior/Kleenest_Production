  ...
  if not exists(
    ...
    where p.id=v_user
      and (
        ...
        or pg_catalog.lower(pg_catalog.coalesce(p.role::text,''::text)) in ('admin','owner','platform_admin','super_admin')
      )
...
begin
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'status_counts',pg_catalog.coalesce((
      ...
      from (
        select d2.status,pg_catalog.count(*)::bigint delivery_count

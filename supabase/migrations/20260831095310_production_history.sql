...
begin
 ...
 return (with peers as(
   select b.id,count(ci.id)::numeric check_ins from public.businesses b
   left join public.locations l on l.business_id=b.id
   left join public.check_ins ci on ci.location_id=l.id and ci.checked_in_at between p_start and p_end
   group by b.id
 ),ranked as(select id,check_ins,percent_rank() over(order by check_ins) percentile from peers),mine as(select * from ranked where id=p_business_id)
 select jsonb_build_object('business_check_ins',coalesce((select check_ins from mine),0),'peer_businesses',(select count(*) from peers),'peer_average_check_ins',coalesce(round((select avg(check_ins) from peers)::numeric,2),0),'percentile',coalesce(round((100*(select percentile from mine))::numeric,2),0)));
end $$;
...
begin
 if not public.business_analytics_authorized(p_business_id) then raise exception 'Business analytics access required'; end if;
 return (select coalesce(jsonb_agg(to_jsonb(x) order by x.created_at desc),'[]'::jsonb) from (
   select p.id,l.name location,p.media_type,p.mime_type,p.size_bytes,p.width,p.height,p.caption,p.storage_path,p.created_at
   from public.location_photos p join public.locations l on l.id=p.location_id
   where l.business_id=p_business_id and p.created_at between p_start and p_end
 ) x);
end $$;
...
begin
 if not public.business_analytics_authorized(p_business_id) then raise exception 'Business analytics access required'; end if;
 return (with v as(select lv.occurred_at,lv.user_id from public.location_visits lv join public.locations l on l.id=lv.location_id where l.business_id=p_business_id and lv.occurred_at between p_start and p_end),
 c as(select ci.checked_in_at occurred_at,ci.user_id from public.check_ins ci join public.locations l on l.id=ci.location_id where l.business_id=p_business_id and ci.checked_in_at between p_start and p_end),
 allv as(select * from v union all select * from c),peak as(select extract(hour from occurred_at)::int h,count(*) n from allv group by 1 order by n desc limit 1)
 select jsonb_build_object('visits',(select count(*) from v),'check_ins',(select count(*) from c),'unique_visitors',(select count(distinct user_id) from allv),'peak_hour',(select h from peak)));
end $$;
...
begin
 if not public.business_analytics_authorized(p_business_id) then raise exception 'Business analytics access required'; end if;
 return (select coalesce(jsonb_agg(to_jsonb(x) order by x.created_at desc),'[]'::jsonb) from (
   select p.id,p.name,p.enabled,p.preferred_access,p.match_discount_bonus,p.custom_perk,p.created_at,count(pa.id) agreements,count(pa.id) filter(where pa.status='active') active_agreements
   from public.partner_programs p left join public.partner_agreements pa on pa.partner_program_id=p.id
   where p.business_id=p_business_id group by p.id,p.name,p.enabled,p.preferred_access,p.match_discount_bonus,p.custom_perk,p.created_at
 ) x);
end $$;

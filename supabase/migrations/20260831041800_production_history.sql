...
begin
  ...
  with eligible as (
    ...
    select p_notification_id,e.token_id,'pending',e.prior_attempts+1,pg_catalog.now() from eligible e
...
revoke all on function internal.claim_native_push_deliveries(uuid,integer) from public,anon,authenticated,service_role;

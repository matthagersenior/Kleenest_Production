...
begin
 for v_job in
  select j.*,l.name location_name,l.business_id
  ...
  order by j.created_at for update skip locked limit greatest(1,least(p_limit,200))
 loop
  begin
   update public.intelligence_notification_jobs set status='processing',attempts=attempts+1 where id=v_job.id;
   select coalesce(s.confidence_score,0)::numeric quality_score,
          coalesce(s.check_in_count,0)::numeric checkins,
          coalesce(s.qr_check_in_count,0)::numeric qr_checkins,
   ...
   into v_signal
   from public.location_feature_summary s
   right join public.locations l on l.id=v_job.location_id
   where s.location_id=v_job.location_id;
   if v_signal is null then
    update public.intelligence_notification_jobs set status='completed',processed_at=now(),last_error='No intelligence feature summary available' where id=v_job.id;
   ...
   if v_type is not null then
    if v_job.surface='business' then
     for v_user in select bm.user_id from public.business_members bm where bm.business_id=v_job.business_id loop
      v_key:=format('intelligence:%s:%s:%s:%s',v_job.surface,v_job.location_id,v_type,v_user);
      select exists(select 1 from public.intelligence_notification_deliveries d where d.user_id=v_user and d.dedupe_key=v_key and d.created_at>now()-interval '120 minutes') into v_recent;
      if not v_recent then
       insert into public.notifications(user_id,type,title,body,data) values(v_user,v_type,v_title,v_body,jsonb_build_object('surface',v_job.surface,'location_id',v_job.location_id,'location_name',v_job.location_name,'dedupe_key',v_key));
       insert into public.intelligence_notification_deliveries(user_id,location_id,surface,notification_type,dedupe_key) values(v_user,v_job.location_id,v_job.surface,v_type,v_key);
      end if;
     end loop;
    elsif v_job.surface='fleet' then
     for v_user in select bm.user_id from public.business_members bm join public.businesses b on b.id=bm.business_id where b.business_tier='fleet' loop
      v_key:=format('intelligence:%s:%s:%s:%s',v_job.surface,v_job.location_id,v_type,v_user);
      select exists(select 1 from public.intelligence_notification_deliveries d where d.user_id=v_user and d.dedupe_key=v_key and d.created_at>now()-interval '120 minutes') into v_recent;
      if not v_recent then
       insert into public.notifications(user_id,type,title,body,data) values(v_user,v_type,v_title,v_body,jsonb_build_object('surface',v_job.surface,'location_id',v_job.location_id,'location_name',v_job.location_name,'dedupe_key',v_key));
   ...
   update public.intelligence_notification_jobs set status='completed',processed_at=now(),last_error=null where id=v_job.id;
   v_processed:=v_processed+1;
  exception when others then
   update public.intelligence_notification_jobs set status=case when attempts>=3 then 'failed' else 'pending' end,available_at=now()+interval '5 minutes',last_error=left(sqlerrm,1000) where id=v_job.id;
  end;
 end loop;
 return v_processed;
end; $$;
...
set search_path to 'public'
as $$
declare v_count integer:=0; r record; v_business_id uuid; v_action text; v_signal_type text;
begin
 for r in
  select j.id,j.location_id,j.surface,
    case
      when exists(select 1 from public.live_network_events e where e.location_id=j.location_id and e.event_type='location.conflict' and e.created_at>now()-interval '2 hours') then 'operational_attention'
      when exists(select 1 from public.live_network_events e where e.location_id=j.location_id and e.created_at>now()-interval '2 hours') then case when j.surface='fleet' then 'high_activity_zone' else 'demand_opportunity' end
  ...
  order by j.created_at asc limit greatest(p_limit,1)
 loop
   select lc.business_id into v_business_id from public.location_claims lc where lc.location_id=r.location_id and lc.status='active' order by lc.created_at desc limit 1;
   v_signal_type:=r.signal_type;
   v_action:=case v_signal_type when 'demand_opportunity' then 'create_promotion' when 'operational_attention' then 'verify_location' when 'high_activity_zone' then 'review_fleet_route' else 'review_intelligence' end;
   insert into public.intelligence_action_links(location_id,business_id,surface,signal_type,action_type,metadata) values(r.location_id,v_business_id,r.surface,v_signal_type,v_action,jsonb_build_object('source_job_id',r.id));
   v_count:=v_count+1;
 end loop;
 return v_count;
end; $$;

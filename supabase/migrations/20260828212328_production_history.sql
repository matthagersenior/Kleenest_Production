create unique index if not exists progression_metric_events_user_metric_idempotency_uidx on public.progression_metric_events (user_id, metric, ((metadata->>'idempotency_key'))) where nullif(metadata->>'idempotency_key','') is not null;
...
begin
  if v_user is null then
    raise exception 'Sign in to contribute an observation.';
  end if;

  if p_location_id is null then
    raise exception 'Canonical location is required.';
  end if;
  ...
  if p_observation_type not in ('clean','dirty','supplies_ok','supplies_low','open','closed','accessible','not_accessible','changing_table','no_changing_table','bathroom_present','bathroom_missing') then
  ...
  if p_cleanliness_pct is not null and (p_cleanliness_pct < 0 or p_cleanliness_pct > 100) then
    raise exception 'Cleanliness must be between 0 and 100.';
  end if;
  ...
  if p_check_in_id is not null then
    ...
    if not found or v_check.user_id <> v_user or v_check.location_id <> p_location_id then
      raise exception 'Check-in does not belong to this user and canonical location.';
    end if;

    if coalesce(v_check.verification_method,'') not in ('gps','qr','place') then
      raise exception 'Check-in is not a verified visit.';
    end if;

    if v_check.checked_in_at > now() + interval '5 minutes' then
      raise exception 'Check-in timestamp is not valid.';
    end if;
    ...
    if exists (
      ...
      where d.user_id = v_user
        ...
        and d.left_at > v_check.checked_in_at
        and d.left_at <= now()
    ) then
      raise exception 'Check-in is no longer an active visit.';
    end if;

    v_confidence := 0.9;
    v_progression_eligible := public.is_qualifying_return_visit(v_user,p_location_id,v_check.checked_in_at,p_check_in_id);
  end if;

  insert into public.restroom_observations(
    location_id,user_id,check_in_id,observation_type,cleanliness_pct,note,source,confidence
  ) values (
    p_location_id,v_user,p_check_in_id,p_observation_type,p_cleanliness_pct,nullif(trim(p_note),''),
    case when p_check_in_id is null then 'community_observation' else 'verified_visit_observation' end,
    v_confidence
  ...
  insert into public.data_feature_events(
    subject_type,subject_id,actor_user_id,location_id,event_type,feature_code,source_table,source_id,
    value_numeric,value_text,metadata,occurred_at,event_validity,confidence,deduplication_key,rate_limit_context
  ) values (
    'location',p_location_id,v_user,p_location_id,'observation','restroom_observation','restroom_observations',v_observation.id,
    p_cleanliness_pct,p_observation_type,
    jsonb_build_object(
      'observation_type',p_observation_type,
      'check_in_id',p_check_in_id,
      'provenance',case when p_check_in_id is null then 'community_observation' else 'verified_visit_observation' end,
  ...
  if v_progression_eligible and p_check_in_id is not null then
    ...
    v_progression := public.record_progression_metric_event(
      'verification',
      'verification',
      v_observation.id,
      1,
      15,
      jsonb_build_object(
        'idempotency_key',v_progression_key,
        'location_id',p_location_id,
        'check_in_id',p_check_in_id,
        'evidence_id',v_observation.id,
        'evidence_type','restroom_observation',
        'server_authoritative',true
      )
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'provenance',case when p_check_in_id is null then 'community_observation' else 'verified_visit_observation' end,
    'progression_eligible',v_progression_eligible,
    'progression_awarded',coalesce(v_progression.points_awarded,0),
    'progression_event_id',v_progression.id
  );
end;
$$;

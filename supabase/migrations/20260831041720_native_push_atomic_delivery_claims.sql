...
begin
  ...
  return query
  with eligible as (
    select t.id as token_id,t.token,t.platform,
...
$$;
revoke all on function internal.claim_native_push_deliveries(uuid,integer) from public,anon,authenticated;
grant execute on function internal.claim_native_push_deliveries(uuid,integer) to service_role;

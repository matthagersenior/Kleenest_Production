...
declare
 ...
 v_input_meta jsonb; v_tags jsonb; v_evidence jsonb; v_source_meta jsonb;
  ...
  begin
   ...
   if loc_id is null then
    ...
    update public.locations l set
      name=coalesce(nullif(l.name,''),v_name),address=coalesce(nullif(l.address,''),v_address),city=coalesce(nullif(l.city,''),v_city),state=coalesce(nullif(l.state,''),v_state),postal_code=coalesce(nullif(l.postal_code,''),v_postal),phone=coalesce(nullif(l.phone,''),v_phone),website=coalesce(nullif(l.website,''),v_website),latitude=coalesce(l.latitude,v_lat),longitude=coalesce(l.longitude,v_lng),place_type=case when coalesce(l.place_type,'place')='place' then v_place_type else l.place_type end,source_dataset=coalesce(l.source_dataset,p_source_key),source_external_id=coalesce(l.source_external_id,ext_id),
      source_metadata=case when v_source_meta='{}'::jsonb then coalesce(l.source_metadata,'{}'::jsonb) else v_source_meta end,
      bathroom_verification_source=case when v_place_type='restroom' then p_source_key else l.bathroom_verification_source end,bathroom_verification_status=case when v_place_type='restroom' then 'has_bathroom' else l.bathroom_verification_status end,updated_at=now()
    where l.id=loc_id;
    updated:=updated+1;
   end if;
   ...
   on conflict(source_id,external_id) do update set location_id=excluded.location_id,latitude=excluded.latitude,longitude=excluded.longitude,name=excluded.name,record_type=excluded.record_type,raw_data='{}'::jsonb,last_seen_at=now(),active=true
...
-- Compact previously retained source metadata to only provenance + verification evidence.
update public.locations l
set source_metadata = jsonb_strip_nulls(jsonb_build_object(
  'provider',nullif(l.source_metadata->>'provider',''),
  'source_dataset',nullif(l.source_metadata->>'source_dataset',''),
  'catalog_dataset_id',nullif(l.source_metadata->>'catalog_dataset_id',''),
  'dataset',nullif(l.source_metadata->>'dataset',''),
  'publisher',nullif(l.source_metadata->>'publisher',''),
  'market_key',nullif(l.source_metadata->>'market_key',''),
  'captured_at',nullif(l.source_metadata->>'captured_at',''),
  'evidence',case
    when jsonb_strip_nulls(jsonb_build_object(
      'amenity',nullif(coalesce(l.source_metadata#>>'{evidence,amenity}',l.source_metadata#>>'{tags,amenity}'),'') ,
      'toilets',nullif(coalesce(l.source_metadata#>>'{evidence,toilets}',l.source_metadata#>>'{tags,toilets}'),'') ,
      'toilets_access',nullif(coalesce(l.source_metadata#>>'{evidence,toilets_access}',l.source_metadata#>>'{tags,toilets:access}'),'') ,
      'wheelchair',nullif(coalesce(l.source_metadata#>>'{evidence,wheelchair}',l.source_metadata#>>'{tags,wheelchair}'),'') ,
      'changing_table',nullif(coalesce(l.source_metadata#>>'{evidence,changing_table}',l.source_metadata#>>'{tags,changing_table}'),'') ,
      'access',nullif(coalesce(l.source_metadata#>>'{evidence,access}',l.source_metadata#>>'{tags,access}'),'')
    ))='{}'::jsonb then null
...
where l.source in ('osm','data_gov') and coalesce(l.source_metadata,'{}'::jsonb) <> '{}'::jsonb;

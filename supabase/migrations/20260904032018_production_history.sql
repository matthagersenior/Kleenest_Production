...
begin
  ...
  if found then return jsonb_build_object('event_id',v_existing.id,'xp_awarded',0,'duplicate',true,'objective_updates','[]'::jsonb); end if;
  if v_cfg.max_per_day is not null then
    ...
    if v_count >= v_cfg.max_per_day then return jsonb_build_object('xp_awarded',0,'withheld',true,'reason','daily_limit','objective_updates','[]'::jsonb); end if;
  end if;
  if p_action like 'discover_%' and v_location is not null and not exists(select 1 from public.discovery_contributions d where d.location_id=v_location and d.user_id=v_user and d.evidence_tier>=v_tier) then raise exception 'discovery evidence does not support requested tier'; end if;
  ...
  values(v_user,'xp_v2','event',v_event,1,v_xp,jsonb_build_object('action',p_action,'evidence_tier',v_tier,'progression_event_v2',true)) on conflict do nothing;
  if v_cfg.specialty is not null then v_specialty := jsonb_build_array(jsonb_build_object('specialty',v_cfg.specialty,'xp_awarded',v_xp)); end if;
  insert into public.user_objective_progress_v2(objective_id,user_id,progress,target,state,completed_at,updated_at)
  select o.id,v_user,least(coalesce((o.rules->>'target')::numeric,1),1),coalesce((o.rules->>'target')::numeric,1),case when coalesce((o.rules->>'target')::numeric,1)<=1 then 'completed' else 'active' end,case when coalesce((o.rules->>'target')::numeric,1)<=1 then now() else null end,now()
  from public.progression_objectives_v2 o where o.status='active' and (o.starts_at is null or o.starts_at<=now()) and (o.ends_at is null or o.ends_at>=now()) and (o.rules->>'action'=p_action or (o.rules->>'action'='discover_any' and p_action like 'discover_%') or coalesce(o.rules->'actions','[]'::jsonb) ? p_action)
  ...
  return jsonb_build_object('event_id',v_event,'base_xp',v_cfg.base_xp,'multiplier',v_mult,'xp_awarded',v_xp,'evidence_tier',v_tier,'global_level',v_level,'specialty_updates',v_specialty,'objective_updates',(select coalesce(jsonb_agg(jsonb_build_object('objective_id',up.objective_id,'progress',up.progress,'target',up.target,'state',up.state)),'[]'::jsonb) from public.user_objective_progress_v2 up join public.progression_objectives_v2 o on o.id=up.objective_id where up.user_id=v_user and (o.rules->>'action'=p_action or (o.rules->>'action'='discover_any' and p_action like 'discover_%') or coalesce(o.rules->'actions','[]'::jsonb) ? p_action)),'duplicate',false);
end $$;

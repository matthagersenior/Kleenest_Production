create or replace function public.business_qr_detail(p_business_id uuid, p_start timestamptz default (now() - interval '30 days'), p_end timestamptz default now()) returns jsonb language plpgsql security definer set search_path='' as $function$
begin
  ...
  if not public.business_can_manage(p_business_id)
     ...
     and not exists(
       ...
       where bm.business_id=p_business_id
         ...
         and lower(bm.role::text) in ('analyst','admin','owner','manager')
     )

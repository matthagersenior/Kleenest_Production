...
BEGIN
  ...
  -- Route mode can expose all discovered ingestion-backed places while
  -- preserving restroom-only behavior for explicit restroom callers.
  v_sample_radius := least(402336, greatest(30000, p_corridor_m + 12000));
  v_sample_count := greatest(1, least(80, ceil(v_route_length / greatest(12000, 1.5 * (v_sample_radius - p_corridor_m)))::integer));
  ...
  scored AS (
    ...
    FROM dedup
    WHERE
      lower(coalesce(nullif(trim(p_category),''),'restroom'))='all'
      OR (
  ...
  SELECT
    ...
    || CASE
      WHEN lower(coalesce(nullif(trim(p_category),''),'restroom'))='restroom'
        THEN jsonb_build_object('category','restroom')
      ELSE '{}'::jsonb
    END
  FROM filtered
  ORDER BY route_pos,route_dist;

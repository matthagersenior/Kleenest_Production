...
begin
  ...
  for item in select value from jsonb_array_elements(p_rows) loop
    ...
    insert into public.data_feature_events(subject_type,subject_id,event_type,feature_code,source_table,source_id,value_numeric,value_text,metadata,occurred_at) values('location',loc_id,'external_data_import','maps.external.'||p_source_key,'external_location_records',rec_id,1,p_source_key,jsonb_build_object('external_id',ext_id,'place_type',place_type,'verification','external_source'),now());
  end loop;

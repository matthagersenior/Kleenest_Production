...
select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'status_conflict',unresolved_conflict,'consensus_status',case when present_count=0 and absent_count=0 then 'unknown' when present_count>=3*greatest(absent_count,1) then 'present' when absent_count>=3*greatest(present_count,1) then 'absent' when unresolved_conflict then 'disputed' when present_count>=absent_count then 'present' else 'absent' end,

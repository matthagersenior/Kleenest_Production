...
create policy notification_native_push_deliveries_self_select on public.notification_native_push_deliveries for select to authenticated using (
  exists (select 1 from public.notification_native_push_tokens t where t.id = token_id and t.user_id = auth.uid())
);

create or replace function public.register_notification_native_push_token(p_token text, p_platform text, p_app_id text default 'com.kleenest.app')
...
begin
  ...
  insert into public.notification_native_push_tokens(user_id,token,provider,platform,app_id,active,updated_at)
  values(auth.uid(),trim(p_token),'expo',p_platform,coalesce(nullif(trim(p_app_id),''),'com.kleenest.app'),true,now())
  on conflict(user_id,token) do update set platform=excluded.platform,app_id=excluded.app_id,active=true,updated_at=now()
...
create or replace function public.remove_notification_native_push_token(p_token text)
returns boolean
language plpgsql
security definer
set search_path = public, auth, pg_temp

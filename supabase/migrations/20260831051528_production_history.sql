...
begin
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'suppressed_30d',pg_catalog.jsonb_build_object(
      'community',count(*) filter(where s.preference_category='community'),
      'rewards',count(*) filter(where s.preference_category='rewards'),
      'intelligence',count(*) filter(where s.preference_category='intelligence'),
      'total',count(s.id)
    )
  ) into v_result
  from (select 1) seed

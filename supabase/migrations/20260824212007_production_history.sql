...
language sql security definer set search_path=public,auth,extensions,pg_temp as $$
...
where n.id=p_network_id and exists (select 1 from public.business_members bm join public.businesses b on b.id=bm.business_id where bm.user_id=auth.uid() and bm.business_id=n.owner_business_id and bm.role in ('owner','admin') and lower(b.business_tier::text) in ('fleet','enterprise'))
...
returns public.enterprise_partner_allocations language plpgsql security definer set search_path=public,auth,extensions,pg_temp as $$
declare a public.enterprise_partner_allocations; begin
if auth.uid() is null then raise exception 'authentication required'; end if;
if not exists (select 1 from public.enterprise_partner_networks n join public.business_members bm on bm.business_id=n.owner_business_id join public.businesses b on b.id=n.owner_business_id where n.id=p_network_id and bm.user_id=auth.uid() and bm.role in ('owner','admin') and lower(b.business_tier::text) in ('fleet','enterprise')) then raise exception 'Fleet or Enterprise network admin access required'; end if;
...
insert into public.enterprise_partner_allocations(network_id,partner_business_id,campaign_id,allocation_type,quantity,budget_cents,rationale) values(p_network_id,p_partner_business_id,p_campaign_id,p_type,greatest(0,p_quantity),greatest(0,p_budget_cents),p_rationale) returning * into a; return a; end $$;

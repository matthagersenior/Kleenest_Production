...
begin
  ...
  if exists(select 1 from public.follows f where f.follower_id = v_user and f.following_id = p_target_user_id) then
    ...
    select coalesce(nullif(trim(p.display_name), ''), nullif(trim(p.username), ''), 'a contributor')
      into v_target_name from public.profiles p where p.id = p_target_user_id;
...
begin
  ...
  select r.user_id,r.location_id into v_review_owner,v_location_id
  from public.reviews r where r.id=p_review_id and r.status='published';

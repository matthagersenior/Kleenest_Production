...
declare
  v_matrix jsonb;
  v_scope text:=split_part(lower(trim(coalesce(p_capability,''))),'.',1);
  v_key text:=split_part(lower(trim(coalesce(p_capability,''))),'.',2);
  v_value text;
begin
...
set search_path=''
as $$
  select public.business_capability_allowed(p_business_id,'enterprise.enabled');
$$;
...
begin
  ...
  if tg_table_name='enterprise_partner_networks' then
    v_business_id:=case when tg_op='DELETE' then old.owner_business_id else new.owner_business_id end;
    v_capability:='enterprise.enterprise_networks';
    ...
    select n.owner_business_id into v_business_id from public.enterprise_partner_networks n where n.id=v_network_id;
    v_capability:='enterprise.enterprise_networks';
  elsif tg_table_name='enterprise_partner_campaigns' then
    v_network_id:=case when tg_op='DELETE' then old.network_id else new.network_id end;
    select n.owner_business_id into v_business_id from public.enterprise_partner_networks n where n.id=v_network_id;
    v_capability:='enterprise.partner_campaigns';
  elsif tg_table_name='enterprise_partner_allocations' then
    v_network_id:=case when tg_op='DELETE' then old.network_id else new.network_id end;
    select n.owner_business_id into v_business_id from public.enterprise_partner_networks n where n.id=v_network_id;
    v_capability:='enterprise.allocations';
    ...
    from public.enterprise_partner_campaigns c join public.enterprise_partner_networks n on n.id=c.network_id
    where c.id=v_campaign_id;
    v_capability:='enterprise.partner_campaigns';
  else
    raise exception 'Unsupported Enterprise guarded table %',tg_table_name;
  end if;

  v_allowed:=public.business_capability_allowed(v_business_id,v_capability)
    and (public.is_platform_owner_session() or exists(
      select 1 from public.business_members bm
      where bm.business_id=v_business_id and bm.user_id=auth.uid() and bm.role in ('owner','admin')
    ));
  if not coalesce(v_allowed,false) then
    raise exception 'Enterprise capability required for %',v_capability;
  end if;
  return case when tg_op='DELETE' then old else new end;
end $$;
revoke all on function public.enterprise_partner_capability_guard() from public,anon,authenticated;

-- Table-level protection closes legacy RPC bypasses without duplicating entitlement logic.
...
create or replace function public.enterprise_list_owned_networks(p_business_id uuid)
returns setof public.enterprise_partner_networks
language sql stable security definer set search_path=''
as $$
  select n.* from public.enterprise_partner_networks n
  where n.owner_business_id=p_business_id
    and public.business_capability_allowed(p_business_id,'enterprise.enterprise_networks')
    and (public.is_platform_owner_session() or exists(select 1 from public.business_members bm where bm.business_id=p_business_id and bm.user_id=auth.uid() and bm.role in ('owner','admin')))
...
as $$
  ...
  where m.network_id=p_network_id
    ...
    and (public.is_platform_owner_session() or exists(select 1 from public.business_members bm where bm.business_id=n.owner_business_id and bm.user_id=auth.uid() and bm.role in ('owner','admin')))
  order by m.created_at;
$$;

create or replace function public.enterprise_list_network_campaigns(p_network_id uuid)
returns setof public.enterprise_partner_campaigns
language sql stable security definer set search_path=''
as $$
  select c.* from public.enterprise_partner_campaigns c
  join public.enterprise_partner_networks n on n.id=c.network_id
  where c.network_id=p_network_id
    and public.business_capability_allowed(n.owner_business_id,'enterprise.partner_campaigns')
    and (public.is_platform_owner_session() or exists(select 1 from public.business_members bm where bm.business_id=n.owner_business_id and bm.user_id=auth.uid() and bm.role in ('owner','admin')))
...
as $$
  select o.partner_business_id,sum(o.visits),sum(o.check_ins),sum(o.reviews),sum(o.preferred_uses),sum(o.access_redemptions),sum(o.promotion_redemptions),sum(o.attributed_users),sum(o.points_awarded),
    round((sum(o.check_ins)*.25+sum(o.reviews)*.15+sum(o.preferred_uses)*.2+sum(o.access_redemptions)*.2+sum(o.promotion_redemptions)*.2)::numeric,2),
    round(case when sum(o.visits)>0 then sum(o.check_ins)::numeric/sum(o.visits)*100 else 0 end,2)
...
as $$
  ...
  from m order by engagement_score desc;
$$;
...
language sql security definer set search_path=''
as $$
  select a.partner_business_id,a.allocation_type,sum(a.budget_cents)::bigint,
    sum(coalesce(o.check_ins,0)+coalesce(o.preferred_uses,0)+coalesce(o.access_redemptions,0)+coalesce(o.promotion_redemptions,0))::bigint,
    case when sum(a.budget_cents)>0 then round((sum(coalesce(o.check_ins,0)+coalesce(o.preferred_uses,0)+coalesce(o.access_redemptions,0)+coalesce(o.promotion_redemptions,0))::numeric*100)/sum(a.budget_cents),4) else 0 end
  ...
  where a.network_id=p_network_id and a.status in('active','completed')
    and public.business_capability_allowed(n.owner_business_id,'enterprise.allocations')
    and (public.is_platform_owner_session() or exists(select 1 from public.business_members bm where bm.business_id=n.owner_business_id and bm.user_id=auth.uid() and bm.role in ('owner','admin')))
...
begin
  ...
  with owned_networks as (
    ...
    select x.network_id,coalesce(sum(x.visits),0)::bigint visits,coalesce(sum(x.check_ins),0)::bigint check_ins,coalesce(sum(x.reviews),0)::bigint reviews,coalesce(sum(x.preferred_uses),0)::bigint preferred_uses,coalesce(sum(x.access_redemptions),0)::bigint access_redemptions,coalesce(sum(x.promotion_redemptions),0)::bigint promotion_redemptions
    from public.enterprise_partner_network_metrics x join owned_networks n on n.id=x.network_id where x.metric_date between v_start and current_date group by x.network_id
  ),outcome_rollup as (
    ...
    from public.enterprise_partner_campaigns c join owned_networks n on n.id=c.network_id left join public.enterprise_partner_campaign_outcomes o on o.campaign_id=c.id and o.metric_date between v_start and current_date group by c.network_id
  )
  select jsonb_build_object('business_id',p_business_id,'window_days',v_days,'start_date',v_start,'end_date',current_date,
    'totals',jsonb_build_object('networks',count(*)::bigint,'enabled_networks',count(*) filter(where n.enabled)::bigint,'active_members',coalesce(sum(m.active_members),0)::bigint,'campaigns',coalesce(sum(c.campaigns),0)::bigint,'active_campaigns',coalesce(sum(c.active_campaigns),0)::bigint,'visits',coalesce(sum(x.visits),0)::bigint,'check_ins',coalesce(sum(x.check_ins),0)::bigint,'reviews',coalesce(sum(x.reviews),0)::bigint,'preferred_uses',coalesce(sum(x.preferred_uses),0)::bigint,'access_redemptions',coalesce(sum(x.access_redemptions),0)::bigint,'promotion_redemptions',coalesce(sum(x.promotion_redemptions),0)::bigint,'attributed_users',coalesce(sum(o.attributed_users),0)::bigint,'points_awarded',coalesce(sum(o.points_awarded),0)::bigint),
    'networks',coalesce(jsonb_agg(jsonb_build_object('id',n.id,'name',n.name,'enabled',n.enabled,'created_at',n.created_at,'active_members',coalesce(m.active_members,0),'campaigns',coalesce(c.campaigns,0),'active_campaigns',coalesce(c.active_campaigns,0),'visits',coalesce(x.visits,0),'check_ins',coalesce(x.check_ins,0),'reviews',coalesce(x.reviews,0),'preferred_uses',coalesce(x.preferred_uses,0),'access_redemptions',coalesce(x.access_redemptions,0),'promotion_redemptions',coalesce(x.promotion_redemptions,0),'attributed_users',coalesce(o.attributed_users,0),'points_awarded',coalesce(o.points_awarded,0)) order by n.created_at desc),'[]'::jsonb),'generated_at',now()) into v_result
  from owned_networks n left join member_rollup m on m.network_id=n.id left join campaign_rollup c on c.network_id=n.id left join metric_rollup x on x.network_id=n.id left join outcome_rollup o on o.network_id=n.id;
...
-- Deterministic acceptance readout for the explicit demo Enterprise tenant.
...
as $$
declare
  b uuid:=p_business_id;
  n uuid;
  result jsonb;
  c_locations integer; c_roles integer; c_qr integer; c_scans integer; c_redemptions integer;
  c_members integer; c_campaigns integer; c_allocations integer; c_remediation integer; c_preventive integer;
  c_vehicles integer; c_drivers integer; c_routes integer; c_active_routes integer; c_alerts integer; c_metrics integer; c_outcomes integer;
begin
...
-- Enterprise reads are authenticated only.
...
-- QR Studio is a managed Business surface; anonymous/public invocation is unnecessary.

create or replace function public.submit_restroom_observation(p_location_id uuid, p_check_in_id uuid, p_observation_type text, p_cleanliness_pct numeric default null, p_note text default null) returns jsonb language plpgsql security definer set search_path='public','auth','extensions','pg_temp' as $function$
declare v_user uuid:=auth.uid(); v_observation public.restroom_observations; v_positive boolean:=false; v_negative boolean:=false; v_confidence numeric:=0.6; v_count integer; v_event_key text; v_check record;
begin
 ...
 if p_check_in_id is not null then
   select id,user_id,location_id,checked_in_at,verification_method into v_check from public.check_ins where id=p_check_in_id;
   if not found or v_check.user_id<>v_user or v_check.location_id<>p_location_id then raise exception 'Check-in does not belong to this user and location.'; end if;
 ...
 v_positive:=p_observation_type in ('clean','supplies_ok','open','accessible','changing_table','bathroom_present'); v_negative:=p_observation_type in ('dirty','supplies_low','closed','not_accessible','no_changing_table','bathroom_missing');
...
comment on function public.submit_restroom_observation(uuid,uuid,text,numeric,text) is 'Records community observations. A linked check-in must belong to the caller/location and use an approved verified-visit method before receiving elevated evidence confidence.';

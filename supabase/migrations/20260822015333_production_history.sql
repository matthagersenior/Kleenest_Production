...
as $$
with base as (
  select l.id,l.place_type,l.name,l.address,l.city,l.state,l.postal_code,l.latitude,l.longitude,l.source,l.source_dataset,l.source_external_id,l.source_metadata,l.rating as l_rating,l.review_count as l_review_count,l.cleanliness_pct,l.verification_confidence,p.id as place_id,p.category as p_category,p.rating as p_rating,p.review_count as p_review_count,p.is_verified as p_is_verified,p.name as place_name,
  ...
  from locations l
  left join places p on p.location_id=l.id and p.is_active=true
  ...
  select * from base where dist<=p_radius_m order by dist limit greatest(1,least(p_limit,500))
), amen as (
  select la.location_id,jsonb_agg(distinct jsonb_build_object('name',a.name,'category',a.category)) items
  from location_amenities la join amenities a on a.id=la.amenity_id group by la.location_id
), fx as (
...
from ranked r left join amen a on a.location_id=r.id left join fx f on f.location_id=r.id
where coalesce(cardinality(p_amenity_names),0)=0
   or exists (select 1 from location_amenities la join amenities aa on aa.id=la.amenity_id where la.location_id=r.id and aa.name=any(p_amenity_names))
order by r.dist;
$$;

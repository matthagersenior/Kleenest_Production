...
begin
  ...
  if 'workforce_wellbeing'=any(v_goals) then v_caps:=v_caps||jsonb_build_array(jsonb_build_object('id','workforce_wellbeing','label','75-user Fleet Premium workforce access','route','/premium','product','fleet','available',v_fleet_available)); end if;

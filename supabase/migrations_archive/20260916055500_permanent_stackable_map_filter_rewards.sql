...
-- Map-filter rewards are permanent capabilities. They stack with every other
-- unlocked map-filter reward and never consume an equipment slot.
...
-- Remove the obsolete either/or equipment state. Unlock authority remains in
-- grants/progression eligibility, so no earned reward is lost.

...
export default function RoutePage() {
  ...
  const checkIn=(stop)=>{if(!navigator.geolocation){setMessage('Location is required to check in.');return;}setStatus('loading');setMessage('Checking your location…');navigator.geolocation.getCurrentPosition(async({coords})=>{try{const result=await checkInAtLocation(stop.id,coords.latitude,coords.longitude);setStatus('ready');setMessage(`Checked in at ${stop.name||'route stop'} · GPS + geofence verified${result?.distance_meters!=null?` · ${Math.round(Number(result.distance_meters))} m away`:''}.`);}catch(err){const detail=String(err?.message||'');setStatus('error');setMessage(detail.includes('OUTSIDE_GEOFENCE')?`Get closer to ${stop.name||'this stop'} to check in. Kleenest verifies GPS check-ins only inside the location geofence.`:detail||'Check-in could not be completed.');}},err=>{setStatus('error');setMessage(err?.message||'Location permission is required to check in.');},{enableHighAccuracy:true,timeout:12000,maximumAge:30000});};

  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div>
          <div className="eyebrow">KLEENEST</div>
          <div className="brand">Find a restroom. Get moving.</div>
        </div>
      </header>
      <main>{children}</main>
  ...
  return (
    <Layout>
      <section className="hero panel">
        ...
        <h1>Find a bathroom without the detour.</h1>
        <p>Kleenest starts with the fastest useful action: find nearby restroom options, choose one, and start navigation.</p>
        <button className="primary" onClick={() => navigate('/nearby')}>Find nearby restrooms</button>
      </section>
      <section className="panel compact">
        <h2>Simple by default</h2>
        <p>Use Explore for one-stop discovery. Open Route when you actually need multiple ordered stops.</p>
      </section>
    </Layout>
  );
}
...
function Nearby() {
  ...
  const load = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      try {
        const rows = await findNearbyRestrooms({ latitude: coords.latitude, longitude: coords.longitude });
        setPlaces(rows);
        setStatus('ready');
      } catch (err) {
      ...
      setError(geoError?.message || 'Location permission is required to find nearby restrooms.');
      setStatus('error');
  ...
  return (
    <Layout>
      <section className="panel">
        ...
        <p>Results come from Kleenest's canonical production map network.</p>
        <button className="primary" onClick={load} disabled={status === 'loading'}>{status === 'loading' ? 'Finding nearby…' : 'Use my location'}</button>
        {status === 'error' && <div className="notice error">{error}</div>}
        {status === 'ready' && places.length === 0 && <div className="notice">No nearby restrooms were returned. Try again from another location.</div>}
      </section>
      <section className="results" aria-live="polite">
        {places.map((place) => {
          const id = place.location_id || place.place_id;
          const miles = Number.isFinite(place.distance_meters) ? `${(place.distance_meters / 1609.344).toFixed(1)} mi` : null;
          return (
            <article className="place-card" key={id || `${place.name}-${place.latitude}-${place.longitude}`}>
              <div className="place-main">
                <h2>{place.name || 'Restroom location'}</h2>

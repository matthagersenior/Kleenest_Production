...
export default function ProfilePage() {
  ...
  useEffect(() => { let active = true; identity.getSession().then((current) => active && refresh(current)).catch((error) => { if (active) { setMessage(error?.message || 'Session could not be loaded.'); setStatus('ready'); } }); const { data } = identity.onAuthStateChange((next) => active && refresh(next)); return () => { active = false; data.subscription.unsubscribe(); }; }, []);

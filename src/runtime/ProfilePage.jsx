...
export default function ProfilePage() {
  ...
  async function refresh(nextSession) {
    ...
    if (!nextSession) { setSummary(null); setStatus('ready'); return; }
    try { setStatus('loading'); setSummary(await getAccountSummary()); setStatus('ready'); }
    catch (error) { setMessage(error?.message || 'Account details could not be loaded.'); setStatus('ready'); }
  }

  useEffect(() => {
    let active = true;
    identity.getSession().then((current) => active && refresh(current)).catch((error) => { if (active) { setMessage(error?.message || 'Session could not be loaded.'); setStatus('ready'); } });

...
export async function getBusinessWorkspaceOverview(businessId){
  ...
  const [access,summary,locations,management,profile,restroomHealth,trustQuality]=await Promise.all([
    ...
    client.rpc('business_restroom_trust_quality',{p_business_id:businessId,p_location_id:null}),
  ]);
...
export async function getFleetWorkspaceOverview(businessId){
  ...
  const client=getSupabase();
  const [authorized,dispatch,summary,signals,exceptions,remediationRisk]=await Promise.all([
  ...
  for(const result of [authorized,dispatch,summary,signals,exceptions,remediationRisk])if(result.error)throw result.error;
  if(!authorized.data)throw new Error('Fleet access is not enabled for this business.');
  return {dispatch:dispatch.data||{},summary:summary.data||{},signals:signals.data||{},exceptions:exceptions.data||{},remediationRisk:remediationRisk.data||{}};
}

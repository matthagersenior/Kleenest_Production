...
export async function checkInAtLocation(locationId, latitude, longitude) {
  const { data, error } = await getSupabase().rpc('kleenest_map_check_in', { p_location_id: locationId, p_lat: latitude, p_lng: longitude });
  if (error) throw error;
  return data;
}
export async function createLocationReview({ locationId, checkInId = null, stars, cleanlinessPct = null, comment = '' }) {
  const { data, error } = await getSupabase().rpc('create_review', { p_location_id: locationId, p_check_in_id: checkInId, p_stars: Number(stars), p_cleanliness_pct: cleanlinessPct == null ? null : Number(cleanlinessPct), p_comment: comment.trim() || null });
  if (error) throw error;
  return data;
}

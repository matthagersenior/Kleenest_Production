...
export type OnboardingTierOption={id:'standard'|'growth'|'fleet'|'enterprise';label:string;price_cents:number|null;billing_unit?:string|null;interval?:string|null;price_note:string|null;max_locations:number|null;premium_users?:number|null;includes?:string[];recommended?:boolean;summary?:string};
export type OnboardingPreview={business_id:string;business_type:string;goals:string[];scale:Record<string,unknown>;current_plan:string;recommended_tier?:'standard'|'growth'|'fleet'|'enterprise';recommended_addons?:string[];tier_reason?:string;tier_options?:OnboardingTierOption[];upgrade_paths?:Record<string,string[]>;enterprise_quote?:Record<string,any>|null;fleet_premium_users?:number;recommended_products:string[];upgrade_required:boolean;needs:Record<string,boolean>;available:Record<string,boolean>;capabilities:Array<{id:string;label:string;route:string;product:string;available:boolean}>;next_steps:Array<{id:string;label:string;route:string;available:boolean}>;answers?:Record<string,unknown>;experience?:Record<string,any>;targeted_routes?:string[];onboarding_version?:number};
...
export async function previewBusinessOnboardingReadOnly(businessId:string,businessType:string,goals:string[],scale:Record<string,number>,answers:Record<string,unknown>={}){return await rpc('business_onboarding_preview_v2',{p_business_id:businessId,p_business_type:businessType,p_goals:goals,p_scale:scale,p_answers:answers}) as OnboardingPreview;}

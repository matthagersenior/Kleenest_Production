...
import { smartDeviceCommand,smartDeviceManifest,smartDeviceSetAutomationRule,upsertSmartDevice,upsertSmartDeviceConnector,type SmartDeviceManifest } from '../services/smartDevices';
...
export default function SmartDevices(){
 const[businessId,setBusinessId]=useState(''),[data,setData]=useState<SmartDeviceManifest|null>(null),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 const[connectorName,setConnectorName]=useState('Facility bridge'),[protocolIndex,setProtocolIndex]=useState(0);
 ...
 async function load(){setBusy(true);try{const id=businessId||await currentBusinessId();setBusinessId(id);setData(await smartDeviceManifest(id));setMessage('')}catch(e:any){setMessage(e?.message||'Smart-device workspace unavailable.')}finally{setBusy(false)}}
 useEffect(()=>{void load()},[]);
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={s.page}>
  ...
  <BusinessCard><SectionHeader title="Connect a bridge" body="Matter, MQTT and vendor clouds connect through a bridge/partner identity. Kleenest never needs a vendor password in the app."/>
   ...
   <Pressable disabled={busy||!businessId||!connectorName.trim()} style={s.primary} onPress={()=>run(()=>upsertSmartDeviceConnector({businessId,name:connectorName.trim(),protocol:protocols[protocolIndex],controlEnabled:false,telemetryEnabled:true}), 'Connector registered. Control stays off until you explicitly enable a paired device.')}><Text style={s.primaryText}>Register connector</Text></Pressable>
  </BusinessCard>
  ...
  <BusinessCard><SectionHeader title="Register a device" body={firstConnector?'Attach a device identity and declare only the capabilities this hardware actually supports.':'Register a connector first.'}/>
   ...
   <Pressable disabled={busy||!firstConnector||!deviceName.trim()||!externalId.trim()} style={s.primary} onPress={()=>run(()=>upsertSmartDevice({businessId,connectorId:String(firstConnector.id),externalDeviceId:externalId.trim(),name:deviceName.trim(),deviceType:deviceType.trim()||'sensor',capabilities:cleanList(capabilities),controlEnabled:cleanList(capabilities).some(x=>x.startsWith('command:')),telemetryEnabled:true}), 'Device registered with least-privilege capabilities.')}><Text style={s.primaryText}>Register device</Text></Pressable>
  </BusinessCard>
  ...
  <View style={s.section}><SectionHeader title="Devices & actions" body="Only commands declared by the device can be queued. High-risk commands are held for KleenestOS approval."/>
   {(data?.devices||[]).map((device:any)=><BusinessCard key={String(device.id)}>
    ...
    <View style={s.actions}>{commandsFor(device).map(command=><Pressable key={command} disabled={busy} style={s.command} onPress={()=>run(()=>smartDeviceCommand({businessId,deviceId:String(device.id),command}),`Command queued: ${command}`)}><Text style={s.commandText}>Send command · {command}</Text></Pressable>)}</View>
   </BusinessCard>)}

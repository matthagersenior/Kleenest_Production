...
export default function BusinessOnboarding(){
 ...
 async function recommend(){if(!goals.length){setMessage('Choose at least one result you want Kleenest to produce.');return}setBusy(true);try{setPreview(await previewBusinessOnboarding(businessId,businessType,goals,scale,answers));setApplied(null);setMessage('Answers saved. Your onboarding draft now shapes Business and Fleet priorities; complete setup when you are ready to apply the starter configuration.')}catch(e:any){setMessage(e?.message||'Recommendation could not be built.')}finally{setBusy(false)}}
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={s.page}>
  ...
  {preview?<View style={s.recommend}><Text style={s.kickerDark}>TARGETED EXPERIENCE</Text><Text style={s.recommendTitle}>{String((preview.experience as any)?.headline||'Your Kleenest operating plan')}</Text>
   <View style={s.tierSummary}><Text style={s.tierEyebrow}>RECOMMENDED TIER</Text><Text style={s.tierName}>{tierDisplay(String(preview.recommended_tier||preview.current_plan||'standard'))}</Text><Text style={s.meta}>{String(preview.tier_reason||'Kleenest matched the tier and add-ons to your operation.')}</Text><Text style={s.tierCurrent}>Current tier: {tierDisplay(String(preview.current_plan||'standard'))}{preview.upgrade_required?' · Upgrade/add-on required':''}</Text>{(preview.recommended_addons||[]).length?<Text style={s.fleetLine}>Recommended add-ons: {(preview.recommended_addons||[]).map(titleCase).join(' + ')}</Text>:null}</View>
   <Text style={s.sub}>Standard → Growth → Fleet → Enterprise</Text><Text style={s.meta}>Fleet is $75/month and includes 50 Premium users, Business Growth tools, and Fleet-specific routing, dispatch, execution, and analytics.</Text><View style={s.tierGrid}>{(preview.tier_options||[]).map(option=><TierCard key={option.id} option={option} recommended={option.id===preview.recommended_tier}/>)}</View>
   {preview.enterprise_quote?<View style={s.enterpriseQuote}><Text style={s.tierEyebrow}>Enterprise pricing</Text><Text style={s.tierName}>{money((preview.enterprise_quote as any).monthly_price_cents)}/month</Text><Text style={s.meta}>Band: {(preview.enterprise_quote as any).band_locations||'250+'} locations · $1,500 onboarding · {(preview.enterprise_quote as any).included_premium_seats||0} Premium seats assumed when Fleet is added.</Text></View>:null}
   <Text style={s.sub}>Upgrade & add-on paths</Text><Text style={s.meta}>{upgradeText(preview)}</Text>
   ...
   <Text style={s.sub}>Capabilities</Text>{(preview.capabilities||[]).map((cap:any)=><View key={cap.id} style={s.cap}><View style={{flex:1}}><Text style={s.capTitle}>{cap.label}</Text><Text style={s.meta}>{cap.product==='fleet'?'FLEET TIER / FLEET ADD-ON':String(cap.product).toUpperCase()}</Text></View><Pill label={cap.available?'AVAILABLE':'OFFER'}/></View>)}
...
function titleCase(value:string){return value?value.charAt(0).toUpperCase()+value.slice(1).toLowerCase():'Standard'}
function tierDisplay(value:string){const v=value.toLowerCase();return v==='fleet'?'Fleet':v==='enterprise'?'Business Enterprise':v==='growth'?'Business Growth':'Business Standard'}
function money(cents:any){return Number.isFinite(Number(cents))?'$'+Math.round(Number(cents)/100).toLocaleString():'Custom'}
function upgradeText(preview:OnboardingPreview){const current=String(preview.current_plan||'standard');const paths=preview.upgrade_paths?.[current]||[];return paths.length?paths.map(x=>x.replaceAll('_',' ')).join(' · '):'Current tier already covers the selected needs.'}
function TierCard({option,recommended}:{option:any;recommended:boolean}){const locationLabel=option.max_locations==null?'Scale-priced':String(option.max_locations)+' location'+(option.max_locations===1?'':'s')+' max';const premium=option.premium_users?String(option.premium_users)+' Premium users':'No included Fleet seats';return <View style={[s.tierCard,recommended&&s.tierCardOn]}><View style={s.tierCardHead}><Text style={[s.tierCardTitle,recommended&&s.tierCardTitleOn]}>{option.label}</Text>{recommended?<Pill label="RECOMMENDED"/>:null}</View><Text style={[s.tierPrice,recommended&&s.tierCardTextOn]}>{String(option.price_note||'Contact')}</Text><Text style={[s.tierCardBody,recommended&&s.tierCardTextOn]}>{Array.isArray(option.includes)?option.includes.join(' · '):''}</Text><Text style={[s.tierCardMeta,recommended&&s.tierCardTextOn]}>{locationLabel} · {premium}</Text></View>}

...
export default function BusinessOnboarding(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={s.page}>
  ...
  {preview?<View style={s.recommend}><Text style={s.kickerDark}>TARGETED EXPERIENCE</Text><Text style={s.recommendTitle}>{String((preview.experience as any)?.headline||'Your Kleenest operating plan')}</Text>
   <View style={s.tierSummary}><Text style={s.tierEyebrow}>RECOMMENDED TIER</Text><Text style={s.tierName}>Business {titleCase(String(preview.recommended_tier||preview.current_plan||'standard'))}</Text><Text style={s.meta}>{String(preview.tier_reason||'Kleenest matched the minimum tier to your scale and selected outcomes.')}</Text><Text style={s.tierCurrent}>Current Business tier: {titleCase(String(preview.current_plan||'standard'))}{preview.upgrade_required?' · Upgrade required':''}</Text>{preview.fleet_included?<Text style={s.fleetLine}>Fleet operations included with this Business tier.</Text>:null}</View>
   <Text style={s.sub}>Standard → Growth → Enterprise</Text><View style={s.tierGrid}>{(preview.tier_options||[]).map(option=><TierCard key={option.id} option={option}/>)}</View>
   ...
   <Text style={s.sub}>Capabilities</Text>{(preview.capabilities||[]).map((cap:any)=><View key={cap.id} style={s.cap}><View style={{flex:1}}><Text style={s.capTitle}>{cap.label}</Text><Text style={s.meta}>{cap.product==='fleet'?('Fleet operations · Business '+titleCase(String(cap.tier||'growth'))):String(cap.product).toUpperCase()}</Text></View><Pill label={cap.available?'AVAILABLE':'OFFER'}/></View>)}
...
function TierCard({option}:{option:any}){const locationLabel=option.max_locations==null?'Portfolio scale':String(option.max_locations)+' location'+(option.max_locations===1?'':'s')+' max';return <View style={[s.tierCard,option.recommended&&s.tierCardOn]}><View style={s.tierCardHead}><Text style={[s.tierCardTitle,option.recommended&&s.tierCardTitleOn]}>{option.label}</Text>{option.recommended?<Pill label="RECOMMENDED"/>:null}</View><Text style={[s.tierPrice,option.recommended&&s.tierCardTextOn]}>{String(option.price_note||'Contact')}</Text><Text style={[s.tierCardBody,option.recommended&&s.tierCardTextOn]}>{option.summary}</Text><Text style={[s.tierCardMeta,option.recommended&&s.tierCardTextOn]}>{locationLabel} · {option.fleet_included?'Fleet operations included':'Core Business'}</Text></View>}

...
export default function BusinessAnalyticsDashboard(){
 ...
 return <ScrollView contentInsetAdjustmentBehavior="automatic" refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  ...
  <View style={[s.summaryCard,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><SectionHeader title="Analytics coverage" body="Every returned analytics domain remains accessible, even when a source has no current-window activity."/><View style={s.coverageRow}><Text style={[s.coverageValue,{color:theme.ink}]}>{sections.length}</Text><Text style={[s.meta,{color:theme.muted}]}>domains available</Text><Text style={[s.coverageValue,{color:theme.ink}]}>{sections.filter(([,v])=>list(v).length||facts(v).length).length}</Text><Text style={[s.meta,{color:theme.muted}]}>with current data</Text></View></View>
  {sections.map(([key,value])=>{const rows=list(value),summary=facts(value);return <View key={key} style={s.section}><SectionHeader title={labels[key]||key.replaceAll('business_','').replaceAll('_',' ')} body={rows.length?String(rows.length)+' current record'+(rows.length===1?'':'s'):'Current-window summary'}/>{summary.length?<BusinessCard>{summary.map(([label,val])=><Fact key={label} label={label} value={val}/>)}</BusinessCard>:null}{rows.length?rows.slice(0,12).map((row,index)=><BusinessCard key={String(row.id||row.location_id||row.campaign_id||index)}><Text style={[s.rowTitle,{color:theme.ink}]}>{summarize(row)}</Text><View style={s.rowFacts}>{facts(row).slice(0,6).map(([label,val])=><Mini key={label} label={label} value={val}/>)}</View></BusinessCard>):!summary.length?<View style={[s.empty,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[s.meta,{color:theme.muted}]}>No current {String(labels[key]||key).toLowerCase()} signal in this window.</Text></View>:null}</View>})}
 </ScrollView>
}
function Fact({label,value}:{label:string;value:string}){const theme=useBusinessTheme();return <View style={[s.fact,{borderBottomColor:theme.line}]}><Text style={[s.factLabel,{color:theme.muted}]}>{label.replaceAll('_',' ')}</Text><Text style={[s.factValue,{color:theme.ink}]}>{value}</Text></View>}
function Mini({label,value}:{label:string;value:string}){const theme=useBusinessTheme();return <View style={[s.mini,{backgroundColor:theme.surfaceRaised}]}><Text style={[s.miniValue,{color:theme.ink}]} numberOfLines={1}>{value}</Text><Text style={[s.miniLabel,{color:theme.muted}]} numberOfLines={1}>{label.replaceAll('_',' ')}</Text></View>}

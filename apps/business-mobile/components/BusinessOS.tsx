...
export function SectionHeader({title,body,actionLabel,onAction}:{title:string;body?:string;actionLabel?:string;onAction?:()=>void}){const theme=useBusinessTheme();
 return <View style={{gap:4}}><View style={s.spread}><Text style={[s.sectionTitle,{color:theme.ink}]}>{title}</Text>{actionLabel&&onAction?<Pressable onPress={onAction}><Text style={[s.actionLink,{color:theme.accent}]}>{actionLabel}</Text></Pressable>:null}</View>{body?<Text style={[s.meta,{color:theme.muted}]}>{body}</Text>:null}</View>
}
...
export function StatCard({label,value,detail,tone='green'}:{label:string;value:string|number;detail?:string;tone?:'green'|'gold'|'teal'|'blue'|'purple'|'coral'}){
 ...
 const theme=useBusinessTheme(); return <BusinessCard style={{flexGrow:1,flexBasis:150,minWidth:145}}><View style={[s.statAccent,{backgroundColor:color}]}/><Text style={[s.statValue,{color:theme.ink}]}>{String(value)}</Text><Text style={[s.statLabel,{color:theme.muted}]}>{label}</Text>{detail?<Text style={[s.meta,{color:theme.muted}]}>{detail}</Text>:null}</BusinessCard>
}
...
export function TrendChart({title,subtitle,values,labels=[]}:{title:string;subtitle?:string;values:number[];labels?:string[]}){const theme=useBusinessTheme();
 ...
 return <BusinessCard><View style={s.spread}><View style={{flex:1}}><Text style={[s.chartTitle,{color:theme.ink}]}>{title}</Text>{subtitle?<Text style={[s.meta,{color:theme.muted}]}>{subtitle}</Text>:null}</View><Text style={[s.delta,delta<0&&{color:businessColors.coral}]}>{delta>0?'+':''}{delta.toLocaleString()}</Text></View><View style={s.chartWrap}><Svg width="100%" height={130} viewBox={`0 0 ${w} 130`}><Line x1="0" y1="112" x2={w} y2="112" stroke={theme.line} strokeWidth="1"/><Path d={path} fill="none" stroke={businessColors.teal} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>{clean.map((v,i)=>{const max=Math.max(...clean,1),min=Math.min(...clean,0),range=Math.max(1,max-min),x=clean.length===1?w/2:(i/(clean.length-1))*w,y=h-((v-min)/range)*h;return <Circle key={i} cx={x} cy={clamp(y,2,h-2)} r={i===clean.length-1?5:3} fill={i===clean.length-1?businessColors.gold:businessColors.teal}/>})}</Svg></View>{labels.length?<View style={s.labelRow}>{labels.slice(-4).map((label,i)=><Text key={`${label}:${i}`} style={s.axisLabel}>{label}</Text>)}</View>:null}</BusinessCard>
}

export function BarChart({title,subtitle,items}:{title:string;subtitle?:string;items:{label:string;value:number}[]}){const theme=useBusinessTheme();
 ...
 return <BusinessCard><Text style={[s.chartTitle,{color:theme.ink}]}>{title}</Text>{subtitle?<Text style={[s.meta,{color:theme.muted}]}>{subtitle}</Text>:null}<View style={{gap:10,marginTop:7}}>{rows.map((row,index)=><View key={row.label} style={{gap:4}}><View style={s.spread}><Text style={[s.barLabel,{color:theme.muted}]}>{row.label}</Text><Text style={[s.barValue,{color:theme.ink}]}>{row.value.toLocaleString()}</Text></View><View style={[s.track,{backgroundColor:theme.surfaceRaised}]}><View style={[s.fill,{width:`${Math.max(3,(Math.max(0,row.value)/max)*100)}%`,backgroundColor:palette[index%palette.length]}]}/></View></View>)}</View></BusinessCard>
}

export function DonutChart({title,value,total,label}:{title:string;value:number;total:number;label:string}){const theme=useBusinessTheme();
 const pct=total>0?clamp(value/total,0,1):0,r=42,circ=2*Math.PI*r,dash=circ*pct;
 return <BusinessCard><Text style={[s.chartTitle,{color:theme.ink}]}>{title}</Text><View style={s.donutRow}><Svg width={118} height={118} viewBox="0 0 118 118"><Circle cx="59" cy="59" r={r} fill="none" stroke={theme.line} strokeWidth="14"/><Circle cx="59" cy="59" r={r} fill="none" stroke={businessColors.green2} strokeWidth="14" strokeLinecap="round" strokeDasharray={`${dash} ${circ-dash}`} rotation="-90" origin="59,59"/></Svg><View style={{flex:1,gap:3}}><Text style={[s.donutValue,{color:theme.ink}]}>{Math.round(pct*100)}%</Text><Text style={[s.statLabel,{color:theme.muted}]}>{label}</Text><Text style={[s.meta,{color:theme.muted}]}>{value.toLocaleString()} of {total.toLocaleString()}</Text></View></View></BusinessCard>
}

export function InsightCallout({title,body,tone='good',actionLabel,onPress}:{title:string;body:string;tone?:'good'|'warning'|'danger';actionLabel?:string;onPress?:()=>void}){const theme=useBusinessTheme();
 const border=tone==='danger'?'#e4bebe':tone==='warning'?'#e7d0a4':'#bfdcc9',background=tone==='danger'?'#fff7f7':tone==='warning'?'#fffaf0':'#f3faf6';
 return <View style={[s.callout,{borderColor:theme.resolved==='dark'?theme.line:border,backgroundColor:theme.resolved==='dark'?theme.surface:background}]}><View style={{flex:1,gap:3}}><Text style={[s.calloutTitle,{color:theme.ink}]}>{title}</Text><Text style={[s.meta,{color:theme.muted}]}>{body}</Text></View>{actionLabel&&onPress?<Pressable onPress={onPress}><Text style={[s.actionLink,{color:theme.accent}]}>{actionLabel}</Text></Pressable>:null}</View>
}

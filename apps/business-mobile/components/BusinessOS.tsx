...
export function DonutChart({title,value,total,label}:{title:string;value:number;total:number;label:string}){const theme=useBusinessTheme();
 ...
 return <BusinessCard><Text style={s.chartTitle}>{title}</Text><View style={s.donutRow}><Svg width={118} height={118} viewBox="0 0 118 118"><Circle cx="59" cy="59" r={r} fill="none" stroke={theme.line} strokeWidth="14"/><Circle cx="59" cy="59" r={r} fill="none" stroke={businessColors.green2} strokeWidth="14" strokeLinecap="round" strokeDasharray={`${dash} ${circ-dash}`} rotation="-90" origin="59,59"/></Svg><View style={{flex:1,gap:3}}><Text style={s.donutValue}>{Math.round(pct*100)}%</Text><Text style={s.statLabel}>{label}</Text><Text style={s.meta}>{value.toLocaleString()} of {total.toLocaleString()}</Text></View></View></BusinessCard>
}
...
export function InsightCallout({title,body,tone='good',actionLabel,onPress}:{title:string;body:string;tone?:'good'|'warning'|'danger';actionLabel?:string;onPress?:()=>void}){const theme=useBusinessTheme();
 ...
 return <View style={[s.callout,{borderColor:theme.resolved==='dark'?theme.line:border,backgroundColor:theme.resolved==='dark'?theme.surface:background}]}><View style={{flex:1,gap:3}}><Text style={s.calloutTitle}>{title}</Text><Text style={s.meta}>{body}</Text></View>{actionLabel&&onPress?<Pressable onPress={onPress}><Text style={s.actionLink}>{actionLabel}</Text></Pressable>:null}</View>
}

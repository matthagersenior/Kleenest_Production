...
export default function DeveloperPartners(){
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={()=>loadDirectory(selectedId)}/>} contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
    <View style={[s.hero,{backgroundColor:theme.accent,borderColor:theme.accent}]}><Text style={[s.eyebrow,{color:theme.accentText}]}>KLEENESTOS · DEVELOPER PLATFORM</Text><Text style={[s.title,{color:theme.accentText}]}>Developer Platform → Partners</Text><Text style={[s.copy,{color:theme.accentText}]}>Create a customer workspace, choose a sellable product bundle, customize API products and quotas, manage team access and credentials, and suspend access from one owner surface.</Text></View>
    ...
    <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
    ...
    <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
      <Text style={[s.sectionTitle,{color:theme.ink}]}>Create partner workspace</Text>
      <TextInput value={newName} onChangeText={setNewName} placeholder="Customer / partner name" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
      <TextInput value={newSlug} onChangeText={setNewSlug} autoCapitalize="none" placeholder="partner-slug" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
      <Text style={[s.label,{color:theme.muted}]}>Product bundle</Text>
      ...
      <Pressable disabled={busy} onPress={()=>void createPartner()} style={[s.primary,busy&&s.disabled]}><Text style={[s.primaryText,{color:theme.accentText}]}>Create Partner</Text></Pressable>
    </View>

    <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.sectionTitle,{color:theme.ink}]}>Partner workspaces</Text><Text style={[s.meta,{color:theme.muted}]}>{partners.length} total · tap one to manage everything.</Text><View style={s.chips}>{sortedPartners.map(p=><Chip key={p.id} text={p.name} on={selectedId===p.id} onPress={()=>{setSelectedId(p.id);void loadDetail(p.id);}}/>)}</View></View>

    {!detail?null:<>
      <View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <View style={s.row}><View style={{flex:1}}><Text style={[s.cardTitle,{color:theme.ink}]}>{detail.partner.name}</Text><Text style={[s.meta,{color:theme.muted}]}>{detail.partner.slug} · {pretty(detail.partner.plan)} · {pretty(detail.partner.status)}</Text></View><Status text={detail.partner.status.toUpperCase()} good={detail.partner.status==='active'}/></View>
        ...
        <Text style={[s.meta,{color:theme.muted}]}>{usagePct}% of monthly quota used.</Text>
        <Pressable onPress={()=>void patch({status:detail.partner.status==='active'?'suspended':'active'},detail.partner.status==='active'?'Partner suspended.':'Partner reactivated.')} style={[s.button,detail.partner.status==='active'&&s.danger]}><Text style={[s.buttonText,{color:theme.accent}]}>{detail.partner.status==='active'?'Suspend partner':'Reactivate partner'}</Text></Pressable>
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.sectionTitle,{color:theme.ink}]}>Product bundle</Text><Text style={[s.meta,{color:theme.muted}]}>Applying a bundle resets plan, default quotas, scopes, API products and integration surfaces. You can override any of those afterward.</Text><View style={s.chips}>{detail.bundles.map(b=><Chip key={b.bundle_key} text={b.label} on={currentBundle===b.bundle_key} onPress={()=>void run(b.label+' applied.',()=>applyDeveloperBundle(selectedId,b.bundle_key))}/>)}</View>{currentBundle==='custom'?<Text style={[s.warning,{color:theme.warning}]}>CUSTOM: this partner has owner overrides beyond a standard bundle.</Text>:null}</View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <Text style={[s.sectionTitle,{color:theme.ink}]}>API products</Text><Text style={[s.meta,{color:theme.muted}]}>These are enforced at request authorization—not merely hidden in the portal.</Text>
        ...
        <Text style={[s.label,{color:theme.muted}]}>Integration surfaces</Text><View style={s.chips}>{surfaces.map(v=><Chip key={v} text={pretty(v)} on={Boolean(access?.integration_surfaces?.includes(v))} onPress={()=>{const next=new Set(access?.integration_surfaces??[]);next.has(v)?next.delete(v):next.add(v);void patch({integration_surfaces:[...next]},'Integration surfaces updated.');}}/>)}</View>
        <Text style={[s.label,{color:theme.muted}]}>Allowed scopes</Text><View style={s.chips}>{scopeOptions.map(v=><Chip key={v} text={v} on={Boolean(access?.scopes?.includes(v))} onPress={()=>{const next=new Set(access?.scopes??[]);next.has(v)?next.delete(v):next.add(v);void patch({scopes:[...next]},'Partner scope ceiling updated.');}}/>)}</View>
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.sectionTitle,{color:theme.ink}]}>Per minute / Per month limits</Text><View style={s.two}><TextInput value={quotaMinute} onChangeText={setQuotaMinute} keyboardType="number-pad" placeholder="Per minute" style={[s.input,{flex:1,backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={quotaMonth} onChangeText={setQuotaMonth} keyboardType="number-pad" placeholder="Per month" style={[s.input,{flex:1,backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/></View><TextInput value={notes} onChangeText={setNotes} placeholder="Owner notes" multiline style={[s.input,{minHeight:70,backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><Pressable onPress={()=>void patch({quota_per_minute:Number(quotaMinute),quota_per_month:Number(quotaMonth),owner_notes:notes},'Quota and owner notes updated.')} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Save Custom Limits</Text></Pressable></View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.sectionTitle,{color:theme.ink}]}>Pilot linkage</Text><Text style={[s.meta,{color:theme.muted}]}>Tie this customer workspace to a named pilot or leave it unlinked for production access.</Text><View style={s.chips}><Chip text="No pilot" on={!access?.pilot_session_id} onPress={()=>void patch({pilot_session_id:null},'Pilot link cleared.')}/>{detail.available_pilots.map(p=><Chip key={p.id} text={p.name+' · '+pretty(p.status)} on={access?.pilot_session_id===p.id} onPress={()=>void patch({pilot_session_id:p.id},'Partner linked to pilot session.')}/>)}</View></View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <Text style={[s.sectionTitle,{color:theme.ink}]}>Team access</Text><Text style={[s.meta,{color:theme.muted}]}>Invite customer developers or change/remove existing workspace roles.</Text>
        <TextInput value={inviteEmail} onChangeText={setInviteEmail} keyboardType="email-address" autoCapitalize="none" placeholder="developer@customer.com" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
        <View style={s.chips}>{roles.map(r=><Chip key={r} text={r} on={inviteRole===r} onPress={()=>setInviteRole(r)}/>)}</View>
        <Pressable onPress={()=>void run('Developer invitation created.',async()=>{const v=await inviteDeveloperMember(selectedId,inviteEmail.trim(),inviteRole);setInviteEmail('');return v;})} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Create Invitation</Text></Pressable>
        {detail.members.map(m=><View key={m.user_id} style={[s.listRow,{borderTopColor:theme.line}]}><View><Text style={[s.rowTitle,{color:theme.ink}]}>{m.email||m.user_id}</Text><Text style={[s.meta,{color:theme.muted}]}>Joined {new Date(m.joined_at).toLocaleDateString()}</Text></View><View style={s.chips}>{roles.map(r=><Chip key={r} text={r} on={m.role===r} onPress={()=>void run('Member role updated.',()=>updateDeveloperMember(selectedId,m.user_id,r))}/>)}</View><Pressable onPress={()=>void run('Member removed.',()=>updateDeveloperMember(selectedId,m.user_id,null))} style={[s.dangerSmall,{backgroundColor:theme.danger+'22'}]}><Text style={[s.dangerText,{color:theme.danger}]}>Remove</Text></Pressable></View>)}
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <Text style={[s.sectionTitle,{color:theme.ink}]}>Credentials & origins</Text><Text style={[s.meta,{color:theme.muted}]}>Server keys use the partner scope ceiling. Browser tokens are origin-bound, expiring and separately rate-limited.</Text>
        <TextInput value={keyLabel} onChangeText={setKeyLabel} placeholder="Server key label" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><Pressable onPress={()=>void run('Server API key issued.',()=>issueDeveloperApiKey(selectedId,keyLabel,access?.scopes??['recommendations:read']))} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Issue Server API Key</Text></Pressable>
        <TextInput value={browserLabel} onChangeText={setBrowserLabel} placeholder="Browser token label" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={browserOrigin} onChangeText={setBrowserOrigin} autoCapitalize="none" placeholder="https://app.customer.com" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
        <View style={s.two}><TextInput value={browserDays} onChangeText={setBrowserDays} keyboardType="number-pad" placeholder="Days" style={[s.input,{flex:1,backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={browserQuota} onChangeText={setBrowserQuota} keyboardType="number-pad" placeholder="Per-minute cap" style={[s.input,{flex:1,backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/></View>
        <Pressable onPress={()=>void run('Browser token issued.',()=>issueDeveloperBrowserToken(selectedId,browserLabel,[browserOrigin.trim()],new Date(Date.now()+Math.max(1,Math.min(30,Number(browserDays)||7))*86400000).toISOString(),Math.max(1,Number(browserQuota)||30)))} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Issue Browser Token</Text></Pressable>
        {detail.api_keys.map(k=><View key={k.id} style={[s.listRow,{borderTopColor:theme.line}]}><View><Text style={[s.rowTitle,{color:theme.ink}]}>{k.label} · {k.credential_type}</Text><Text style={[s.meta,{color:theme.muted}]}>{k.key_prefix} · {k.scopes.join(', ')}</Text>{k.allowed_origins?.length?<Text style={[s.meta,{color:theme.muted}]}>{k.allowed_origins.join(', ')}</Text>:null}<Text style={[s.meta,{color:theme.muted}]}>{k.revoked_at?'REVOKED':'Last used '+(k.last_used_at?new Date(k.last_used_at).toLocaleString():'never')}</Text></View>{!k.revoked_at?<Pressable onPress={()=>void run('Credential revoked.',()=>revokeDeveloperCredential(selectedId,k.id))} style={[s.dangerSmall,{backgroundColor:theme.danger+'22'}]}><Text style={[s.dangerText,{color:theme.danger}]}>Revoke</Text></Pressable>:null}</View>)}
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <Text style={[s.sectionTitle,{color:theme.ink}]}>Webhooks</Text><TextInput value={webhookUrl} onChangeText={setWebhookUrl} autoCapitalize="none" placeholder="https://customer.com/webhooks/kleenest" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={webhookEvents} onChangeText={setWebhookEvents} placeholder="place.updated,place.verification_changed" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
        <Pressable onPress={()=>void run('Webhook created.',()=>createDeveloperWebhook(selectedId,webhookUrl.trim(),'Kleenest webhook',webhookEvents.split(',').map(v=>v.trim()).filter(Boolean)))} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Create Webhook</Text></Pressable>
        {detail.webhooks.map(w=><View key={w.id} style={[s.listRow,{borderTopColor:theme.line}]}><View><Text style={[s.rowTitle,{color:theme.ink}]}>{w.label} · {w.active?'ACTIVE':'DISABLED'}</Text><Text style={[s.meta,{color:theme.muted}]}>{w.url}</Text><Text style={[s.meta,{color:theme.muted}]}>{w.event_types.join(', ')} · failures {w.consecutive_failures}</Text></View>{w.active?<Pressable onPress={()=>void run('Webhook disabled.',()=>disableDeveloperWebhook(selectedId,w.id))} style={[s.dangerSmall,{backgroundColor:theme.danger+'22'}]}><Text style={[s.dangerText,{color:theme.danger}]}>Disable</Text></Pressable>:null}</View>)}
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}>
        <Text style={[s.sectionTitle,{color:theme.ink}]}>Billing</Text><View style={s.chips}>{['manual','stripe','shopify','other'].map(v=><Chip key={v} text={v} on={billingProvider===v} onPress={()=>setBillingProvider(v)}/>)}</View>
        <TextInput value={billingStatus} onChangeText={setBillingStatus} placeholder="subscription status" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={billingPlan} onChangeText={setBillingPlan} placeholder="plan code" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={billingCustomer} onChangeText={setBillingCustomer} placeholder="external customer id" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><TextInput value={billingSubscription} onChangeText={setBillingSubscription} placeholder="external subscription id" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
        <Pressable onPress={()=>void run('Billing state updated.',()=>setDeveloperPartnerBilling(selectedId,{provider:billingProvider,status:billingStatus,planCode:billingPlan||null,externalCustomerId:billingCustomer||null,externalSubscriptionId:billingSubscription||null}))} style={[s.button,{backgroundColor:theme.accentSoft}]}><Text style={[s.buttonText,{color:theme.accent}]}>Save Billing</Text></Pressable>
      </View>

      <View style={[s.section,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.sectionTitle,{color:theme.ink}]}>Audit trail</Text><Text style={[s.meta,{color:theme.muted}]}>Owner changes to bundle, products, quotas, team, credentials, webhooks and billing are retained here.</Text>{detail.control_log.length===0?<Text style={[s.meta,{color:theme.muted}]}>No owner control events recorded yet.</Text>:detail.control_log.map(a=><View key={a.id} style={[s.audit,{borderTopColor:theme.line}]}><Text style={[s.rowTitle,{color:theme.ink}]}>{pretty(a.action)}</Text><Text style={[s.meta,{color:theme.muted}]}>{new Date(a.created_at).toLocaleString()} · {a.reason||'No reason recorded'}</Text></View>)}</View>
    </>}
  </ScrollView>;
}

function Metric({label,value}:{label:string;value:string}){const theme=usePlatformTheme();return <View style={[s.metric,{backgroundColor:theme.surfaceRaised}]}><Text style={[s.metricValue,{color:theme.accent}]}>{value}</Text><Text style={[s.meta,{color:theme.muted}]}>{label}</Text></View>}

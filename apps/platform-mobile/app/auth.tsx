...
export default function OwnerAuth() {
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[s.page,{backgroundColor:theme.canvas},{backgroundColor:theme.canvas}]}>
    <View style={[s.hero,{backgroundColor:theme.accent,borderColor:theme.line},{backgroundColor:theme.accent,borderColor:theme.line}]}><Text style={[s.eyebrow,{color:theme.accentText},{color:theme.accentText}]}>KLEENESTOS · PRIVATE CONTROL PLANE</Text><Text style={[s.heroTitle,{color:theme.accentText},{color:theme.accentText}]}>{heroTitle}</Text><Text style={[s.heroBody,{color:theme.accentText},{color:theme.accentText}]}>{heroBody}</Text></View>
    ...
    {notice ? <View style={[s.notice,{backgroundColor:theme.surface,borderColor:theme.line},{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={s.noticeText}>{notice}</Text></View> : null}
    ...
    {!recovering ? <View style={s.field}><Text style={[s.label,{color:theme.muted},{color:theme.muted}]}>Owner email</Text><TextInput accessibilityLabel="Owner email" value={email} onChangeText={setEmail} autoCapitalize="none" autoCorrect={false} keyboardType="email-address" autoComplete="email" textContentType="emailAddress" placeholder="owner@example.com" placeholderTextColor="#7f8d85" selectionColor="#132b21" cursorColor="#132b21" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink},{backgroundColor:theme.surfaceRaised,borderColor:theme.line},{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}/></View> : null}
    {!forgetting ? <View style={s.field}><Text style={[s.label,{color:theme.muted},{color:theme.muted}]}>{recovering ? 'New Owner password' : 'Owner password'}</Text><View style={[s.passwordRow,{backgroundColor:theme.surfaceRaised,borderColor:theme.line},{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><TextInput accessibilityLabel={recovering ? 'New Owner password' : 'Owner password'} value={password} onChangeText={setPassword} secureTextEntry={!showPassword} autoCapitalize="none" autoCorrect={false} autoComplete={creating || recovering ? 'new-password' : 'current-password'} textContentType={creating || recovering ? 'newPassword' : 'password'} placeholder={recovering ? 'Create a new password' : creating ? 'Create a password' : 'Enter owner password'} placeholderTextColor="#7f8d85" selectionColor="#132b21" cursorColor="#132b21" style={s.passwordInput}/><Pressable accessibilityRole="button" accessibilityLabel={showPassword ? 'Hide owner password' : 'Show owner password'} onPress={() => setShowPassword(v => !v)} style={[s.visibility,{backgroundColor:theme.surfaceRaised,borderColor:theme.line},{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={s.visibilityText}>{showPassword ? 'Hide' : 'Show'}</Text></Pressable></View></View> : null}
    {creating || recovering ? <View style={s.field}><Text style={[s.label,{color:theme.muted},{color:theme.muted}]}>Confirm password</Text><TextInput accessibilityLabel="Confirm owner password" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showPassword} autoCapitalize="none" autoCorrect={false} autoComplete="new-password" textContentType="newPassword" placeholder="Re-enter password" placeholderTextColor="#7f8d85" selectionColor="#132b21" cursorColor="#132b21" style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink},{backgroundColor:theme.surfaceRaised,borderColor:theme.line},{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}/></View> : null}
...
function ModeButton({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  const theme=usePlatformTheme(); return <Pressable onPress={onPress} accessibilityRole="button" accessibilityState={{ selected: active }} style={[s.modeButton, active && s.modeActive]}><Text style={[s.modeText, active && s.modeTextActive]}>{label}</Text></Pressable>; }

...
export default function Account(){
  ...
  const[email,setEmail]=useState(''),[password,setPassword]=useState(''),[showPassword,setShowPassword]=useState(false),[signedIn,setSignedIn]=useState(''),[authority,setAuthority]=useState(''),[reason,setReason]=useState(''),[message,setMessage]=useState(''),[busy,setBusy]=useState(false);
  const client=getKleenestSupabaseClient();
  const mobileAuthRedirect=Linking.createURL('/account',{scheme:'kleenest-owner'});
  ...
  async function refresh(){
    ...
    if(data.user){
      try{
        const a=await getOwnerAuthorization();
        setAuthority(a.is_platform_owner?'Platform Owner':a.is_admin?'Administrator':'Unauthorized');
      }catch{setAuthority('Unauthorized')}
    }else setAuthority('');
  }
  ...
  async function verifyOwnerAccess(){
    try{
      ...
      if(!a.authorized){
        ...
        setMessage('This account is not authorized for the Kleenest Owner control plane.');
        await refresh();
        return false;
      }
      setMessage(`Signed in with ${a.is_platform_owner?'Platform Owner':'Administrator'} authority.`);
      await refresh();
      return true;
    }catch(error:any){
      await client.auth.signOut({scope:'local'});
      setMessage(error?.message||'Owner authorization could not be verified.');
      await refresh();
      return false;
    }
  }
  ...
  async function handleAuthUrl(url:string|null){
    ...
    try{
      ...
      return await verifyOwnerAccess();
    }catch(error:any){
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
    ...
    {!signedIn?<View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}>
      ...
      <Pressable style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line},busy&&s.disabled]} disabled={busy} onPress={googleSignIn}><Text style={[s.secondaryText,{color:theme.accent}]}>Continue with Google</Text></Pressable>
    </View>:<Pressable style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]} onPress={signOut}><Text style={[s.secondaryText,{color:theme.accent}]}>Sign out</Text></Pressable>}
    <View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.body,{color:theme.muted}]}>Administrator access does not bypass personal account controls.</Text><TextInput style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]} placeholderTextColor={theme.muted} placeholder="Optional deletion reason" value={reason} onChangeText={setReason}/><Pressable style={[s.danger,{backgroundColor:theme.danger}]} onPress={deletion}><Text style={[s.dangerText,{color:theme.resolved==='dark'?theme.canvas:'#fff'}]}>Request account deletion</Text></Pressable></View>
    {message?<Text accessibilityLiveRegion="polite" style={[s.body,{color:theme.warning}]}>{message}</Text>:null}
  </ScrollView>
}

const s=StyleSheet.create({themeCard:{padding:14,borderRadius:18,borderWidth:1,gap:8},themeHeading:{fontSize:16,fontWeight:'900'},themeCopy:{fontSize:12,lineHeight:18},themeRow:{flexDirection:'row',flexWrap:'wrap',gap:7},themeButton:{paddingHorizontal:11,paddingVertical:9,borderRadius:999,borderWidth:1},themeButtonText:{fontSize:11,fontWeight:'900'},page:{padding:20,gap:12,backgroundColor:'#f3f6f4'},title:{fontSize:28,fontWeight:'900',color:'#102218'},body:{fontSize:14,lineHeight:21,color:'#56665d'},card:{backgroundColor:'#fff',padding:16,borderRadius:18,borderWidth:1,borderColor:'#d5e0d9',gap:9},input:{borderWidth:1,borderColor:'#cbd9d0',borderRadius:12,padding:12},passwordRow:{borderWidth:1,borderColor:'#cbd9d0',borderRadius:12,flexDirection:'row',alignItems:'center',overflow:'hidden'},eye:{paddingHorizontal:13,paddingVertical:12},eyeText:{fontWeight:'900',color:'#173d2b'},primary:{backgroundColor:'#173d2b',padding:12,borderRadius:12,alignItems:'center'},primaryText:{color:'#fff',fontWeight:'900'},secondary:{backgroundColor:'#edf3ef',borderWidth:1,borderColor:'#d5e0d9',padding:12,borderRadius:12,alignItems:'center'},secondaryText:{color:'#173d2b',fontWeight:'900'},or:{textAlign:'center',color:'#7a8a80',fontSize:12,fontWeight:'700'},disabled:{opacity:.55},danger:{backgroundColor:'#7b2f2f',padding:12,borderRadius:12,alignItems:'center'},dangerText:{color:'#fff',fontWeight:'900'}});

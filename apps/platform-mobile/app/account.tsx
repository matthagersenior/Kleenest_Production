...
export default function Account(){
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
    <Text style={s.title}>Administrator account</Text>
    <Text style={s.body}>{signedIn?`Signed in as ${signedIn}${authority?` · ${authority}`:''}`:'Sign in with an authorized Kleenest Platform administrator account.'}</Text>
    ...
    <View style={s.card}><Text style={s.body}>Administrator access does not bypass personal account controls.</Text><TextInput style={s.input} placeholder="Optional deletion reason" value={reason} onChangeText={setReason}/><Pressable style={s.danger} onPress={deletion}><Text style={s.dangerText}>Request account deletion</Text></Pressable></View>

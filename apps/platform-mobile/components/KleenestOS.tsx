...
export function SectionHeader({title,body,actionLabel,onAction}:{title:string;body?:string;actionLabel?:string;onAction?:()=>void}){const theme=usePlatformTheme();return <View style={{gap:4}}><View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8}}><Text style={{fontSize:19,fontWeight:'900',color:theme.ink,flex:1}}>{title}</Text>{actionLabel&&onAction?<Pressable onPress={onAction}><Text style={{fontWeight:'900',color:theme.accent}}>{actionLabel}</Text></Pressable>:null}</View>{body?<Text style={{color:theme.muted,lineHeight:19}}>{body}</Text>:null}</View>}

export function DiagnosticDisclosure({title,value}:{title:string;value:unknown}){const[open,setOpen]=useState(false);const theme=usePlatformTheme();const card=useOSCardStyle();return <View style={card}><Pressable onPress={()=>setOpen(value=>!value)}><Text style={{fontWeight:'900',color:theme.accent}}>{open?'Hide':'Show'} {title}</Text></Pressable>{open?<Text selectable style={{fontFamily:'monospace',fontSize:11,color:theme.muted}}>{JSON.stringify(value,null,2)}</Text>:null}</View>}

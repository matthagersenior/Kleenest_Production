...
export async function resolveOwnerLocationClaim(claimId:string,status:'approved'|'rejected',note?:string){await requirePlatformOwner();const{data,error}=await client().rpc('admin_resolve_location_claim_v2',{p_claim_id:claimId,p_status:status,p_note:note?.trim()||null});if(error)throw new Error(error.message);return data;}

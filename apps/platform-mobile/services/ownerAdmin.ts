...
export async function getOwnerModerationQueues(){await requirePlatformOwner();const[photoReports,reviewReports,userReports,aiReports,pendingBusinesses]=await Promise.all([client().rpc('admin_list_review_photo_reports',{p_status:null}),client().rpc('admin_list_review_reports',{p_status:'pending'}),client().rpc('admin_list_user_safety_reports',{p_status:'open'}),client().rpc('admin_list_ai_response_reports',{p_status:'open'}),client().rpc('admin_list_pending_businesses')]);for(const result of[photoReports,reviewReports,userReports,aiReports,pendingBusinesses])if(result.error)throw result.error;const photos=asRows(photoReports.data).filter(row=>['open','reviewing'].includes(String(row.status||''))).map(row=>({...row,public_url:row.storage_path?client().storage.from('review-photos').getPublicUrl(String(row.storage_path)).data.publicUrl:null}));return{photoReports:photos,reviewReports:asRows(reviewReports.data),userReports:asRows(userReports.data),aiReports:asRows(aiReports.data),pendingBusinesses:asRows(pendingBusinesses.data)};}

...
export default function LegalCenter(){
  ...
  if(loading)return <View style={[center,{backgroundColor:theme.canvas}]}><ActivityIndicator size="large" color={theme.accent}/></View>;
  return <ScrollView contentContainerStyle={[page,{backgroundColor:theme.canvas}]}>
    <View style={[hero,{backgroundColor:theme.accent}]}><Text style={[eyebrow,{color:theme.accentText,opacity:.78}]}>SAFETY + LEGAL</Text><Text style={[title,{color:theme.accentText}]}>Kleenest policies</Text><Text style={[heroBody,{color:theme.accentText,opacity:.88}]}>These policies protect the trust network, contributors, businesses, and location data that Kleenest depends on.</Text></View>
    {error?<View style={[errorCard,{backgroundColor:theme.surfaceRaised,borderColor:theme.danger}]}><Text style={[errorText,{color:theme.danger}]}>{error}</Text></View>:null}
    ...
    <View style={[card,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[heading,{color:theme.ink}]}>Your controls</Text><Text style={[body,{color:theme.muted}]}>Report unsafe content or contributors from their content/profile. Blocked contributors cannot continue direct interaction with you. You can manage blocked users and account deletion from Profile.</Text><Link href="/blocked-users" asChild><Pressable accessibilityRole="link" accessibilityLabel="Manage blocked users" style={[secondary,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[secondaryText,{color:theme.accent}]}>Manage blocked users</Text></Pressable></Link><Link href="/account-deletion" asChild><Pressable accessibilityRole="link" accessibilityLabel="Account deletion" style={[secondary,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[secondaryText,{color:theme.accent}]}>Account deletion</Text></Pressable></Link></View>

...
export default function GameArena(){
 const appTheme=useConsumerTheme();
 ...
 if(loading)return <SafeAreaView style={[s.safe,{backgroundColor:appTheme.canvas}]}><View style={s.loading}><Text style={s.loadingGlyph}>{theme.glyph}</Text><Text style={[s.loadingText,{color:appTheme.ink}]}>Building a fresh {game.name} run…</Text></View></SafeAreaView>;

 return <SafeAreaView style={[s.safe,{backgroundColor:appTheme.canvas}]}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
  <View style={[s.hero,{backgroundColor:appTheme.resolved==='dark'?appTheme.surfaceRaised:theme.surface,borderColor:appTheme.line}]}>
   <View style={s.topRow}><Pressable onPress={()=>router.back()} style={s.back}><Text style={[s.backText,{color:appTheme.resolved==='dark'?appTheme.muted:appTheme.accentText}]}>‹ ARCADE</Text></Pressable><Text style={[s.arenaLabel,{color:theme.accent}]}>{theme.label}</Text></View>
   <Text style={s.glyph}>{theme.glyph}</Text><Text style={[s.title,{color:appTheme.resolved==='dark'?appTheme.ink:appTheme.accentText}]}>{game.name}</Text><Text style={[s.tagline,{color:appTheme.resolved==='dark'?appTheme.muted:appTheme.accentText}]}>{theme.tagline}</Text>
   ...
   <View style={s.heatRow}><Text style={[s.heat,{color:theme.accent}]}>{heat}</Text><Text style={[s.heroMeta,{color:appTheme.resolved==='dark'?appTheme.muted:appTheme.accentText}]}>Round {Math.min(round+1,game.rounds)} / {game.rounds}</Text>{usesLives?<Text style={s.heroMeta}>{'♥'.repeat(lives)}{'♡'.repeat(Math.max(0,3-lives))}</Text>:null}{game.timeLimitSec&&!complete?<Text style={[s.clock,{color:theme.accent}]}>{timeLeft}s</Text>:null}</View>
  </View>

  {game.mode==='strategy'?<View style={[s.resourceCard,{backgroundColor:appTheme.surface,borderColor:appTheme.line}]}><Text style={[s.resourceLabel,{color:appTheme.muted}]}>EVIDENCE BUDGET</Text><Text style={[s.resourceValue,{color:appTheme.accent}]}>{strategyTokens}</Text><Text style={[s.resourceBody,{color:appTheme.muted}]}>tokens left · stronger verification moves cost more</Text></View>:null}

  <View style={[s.arenaCard,{backgroundColor:appTheme.surface,borderColor:appTheme.line}]}>
   <View style={s.challengeHead}><Text style={[s.challengeKicker,{color:appTheme.muted}]}>{theme.label} · {game.difficulty.toUpperCase()}</Text><Text style={[s.combo,{color:appTheme.warning}]}>{combo>=2?combo+'× COMBO':'BUILD A COMBO'}</Text></View>
   {complete?<Results/>:game.mode==='memory'?<Memory/>:game.mode==='builder'?<Builder/>:<Choice/>}
   <Text accessibilityLiveRegion="polite" style={[s.message,{color:appTheme.muted}]}>{message}</Text>
  </View>

  <View style={[s.masteryCard,{backgroundColor:appTheme.surface,borderColor:appTheme.line}]}><Text style={[s.masteryKicker,{color:appTheme.muted}]}>MASTERY</Text><Text style={[s.masteryTitle,{color:appTheme.accent}]}>{mastery}</Text><Text style={[s.masteryBody,{color:appTheme.muted}]}>Personal best {bestScore} · {Number(record?.plays||0)} prior run{Number(record?.plays||0)===1?'':'s'}. XP is progression; this is the score you come back to beat.</Text></View>
  <View style={[s.challengeCard,{backgroundColor:appTheme.surface,borderColor:appTheme.line}]}><Text style={[s.masteryKicker,{color:appTheme.muted}]}>CHALLENGE THIS GAME</Text><Text style={[s.challengeTitle,{color:appTheme.ink}]}>Put your run against someone you follow.</Text><Text style={[s.challengeBody,{color:appTheme.muted}]}>They get the same game and score ceiling. Fresh scenarios can differ, so the match rewards mastery rather than memorizing one fixed question order.</Text>{challengeMessage?<Text style={[s.challengeNotice,{color:appTheme.accent}]}>{challengeMessage}</Text>:null}<View style={s.targetList}>{targets.slice(0,6).map(target=><View key={String(target.user_id)} style={[s.targetRow,{borderTopColor:appTheme.line}]}><Pressable style={{flex:1}} onPress={()=>router.push({pathname:'/contributor/[id]',params:{id:String(target.user_id)}})}><Text style={[s.targetName,{color:appTheme.ink}]}>{target.display_name||target.username||'Contributor'}</Text><Text style={[s.targetMeta,{color:appTheme.muted}]}>{target.relationship||'community'} · view profile →</Text></Pressable><Pressable style={[s.secondary,{backgroundColor:appTheme.accentSoft,borderColor:appTheme.line}]} onPress={async()=>{try{await createGameChallenge(game.code,String(target.user_id));setChallengeMessage('Challenge sent to '+(target.display_name||target.username||'player')+'.')}catch(error:any){setChallengeMessage(error?.message||'Challenge could not be sent.')}}}><Text style={[s.secondaryText,{color:appTheme.accent}]}>CHALLENGE</Text></Pressable></View>)}</View>{!targets.length?<Text style={[s.challengeBody,{color:appTheme.muted}]}>Follow contributors in Community to unlock rivals.</Text>:null}</View>
 </ScrollView></SafeAreaView>;

 function Choice(){
  if(!current)return <Text style={[s.question,{color:appTheme.ink}]}>No fresh round is available.</Text>;
  return <><Text style={[s.context,{color:appTheme.muted}]}>{current.stage||current.context||game.instructions}</Text><Text style={[s.question,{color:appTheme.ink}]}>{current.prompt}</Text>
   {current.routeMetrics?<View style={s.routeGrid}>{current.routeMetrics.map(route=><View key={route.label} style={[s.routeCard,{backgroundColor:appTheme.surfaceRaised,borderColor:appTheme.line}]}><Text style={[s.routeName,{color:appTheme.ink}]}>{route.label}</Text><Text style={[s.routeMeta,{color:appTheme.muted}]}>{route.distance} · freshness {route.freshness}</Text><Text style={[s.routeMeta,{color:appTheme.muted}]}>{route.verified?'✓ verified':'○ unverified'} · {route.accessible?'♿ accessible':'standard access'}{route.neededAmenity?' · ✓ needed amenity':''}</Text></View>)}</View>:null}
   <View style={s.answers}>{current.choices.map((choice,index)=>{const cost=current.costs?.[index];return <Pressable key={choice} onPress={()=>answer(index)} style={[s.answer,{backgroundColor:appTheme.surfaceRaised,borderColor:appTheme.line}]}><View style={[s.answerBadge,{backgroundColor:theme.surface}]}><Text style={s.answerBadgeText}>{game.mode==='ranking'?index+1:String.fromCharCode(65+index)}</Text></View><Text style={[s.answerText,{color:appTheme.ink}]}>{choice}</Text>{cost?<Text style={[s.cost,{color:appTheme.warning}]}>{cost}◈</Text>:null}</Pressable>})}</View>
  </>;
 }
 function Builder(){
  if(!builder)return null;
  return <><Text style={s.context}>BUILD LAB · SELECT EVERY SUPPORTED AMENITY</Text><Text style={[s.question,{color:appTheme.ink}]}>{builder.prompt}</Text><View style={s.answers}>{builder.options.map((option:string,index:number)=><Pressable key={option} onPress={()=>toggleBuilder(index)} style={[s.answer,{backgroundColor:appTheme.surfaceRaised,borderColor:appTheme.line},builderSelected.includes(index)&&{backgroundColor:appTheme.accent,borderColor:appTheme.accent}]}><Text style={[s.answerText,{color:appTheme.ink},builderSelected.includes(index)&&{color:appTheme.accentText}]}>{builderSelected.includes(index)?'✓ ':''}{option}</Text></Pressable>)}</View><Pressable style={[s.primary,{backgroundColor:appTheme.accent}]} onPress={submitBuilder}><Text style={[s.primaryText,{color:appTheme.accentText}]}>LOCK PROFILE</Text></Pressable></>;
 }
 function Memory(){
  return <><Text style={s.context}>MEMORY GRID · {memoryMoves} MOVES</Text><Text style={[s.question,{color:appTheme.ink}]}>Match each restroom condition to the evidence that proves it.</Text><View style={s.memoryGrid}>{memoryCards.map((card,index)=>{const shown=memoryOpen.includes(index)||memoryMatched.includes(index);return <Pressable key={index} onPress={()=>flipMemory(index)} style={[s.memoryCard,{backgroundColor:appTheme.surfaceRaised,borderColor:appTheme.line},memoryMatched.includes(index)&&{backgroundColor:appTheme.accent,borderColor:appTheme.accent}]}><Text style={[s.memoryText,{color:appTheme.ink},memoryMatched.includes(index)&&{color:appTheme.accentText}]}>{shown?card.text:'?'}</Text></Pressable>})}</View></>;
 }
 function Results(){
  return <View style={s.results}><Text style={[s.resultsKicker,{color:appTheme.muted}]}>{lives<=0?'RUN ENDED':'RUN COMPLETE'}</Text><Text style={[s.resultsScore,{color:appTheme.ink}]}>{score}</Text><Text style={[s.resultsLabel,{color:appTheme.muted}]}>FINAL SCORE</Text><View style={s.resultStats}><Result value={accuracy+'%'} label="accuracy"/><Result value={maxCombo+'×'} label="best combo"/><Result value={mastery} label="mastery"/></View><Pressable disabled={saved} style={[s.primary,{backgroundColor:appTheme.accent},saved&&s.disabled]} onPress={save}><Text style={s.primaryText}>{saved?'RUN SAVED':'SAVE RUN'}</Text></Pressable><View style={s.resultActions}><Pressable style={[s.secondary,{backgroundColor:appTheme.accentSoft,borderColor:appTheme.line}]} onPress={reset}><Text style={s.secondaryText}>PLAY AGAIN</Text></Pressable><Pressable style={s.secondary} onPress={()=>router.push('/games')}><Text style={s.secondaryText}>ARCADE</Text></Pressable><Pressable style={s.secondary} onPress={()=>router.push('/progress')}><Text style={s.secondaryText}>LEAGUE</Text></Pressable></View></View>;
 }
}
function Hud({value,label}:{value:any;label:string}){const theme=useConsumerTheme();return <View style={[s.hudCell,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.hudValue,{color:theme.ink}]}>{value}</Text><Text style={[s.hudLabel,{color:theme.muted}]}>{label}</Text></View>}
function Result({value,label}:{value:any;label:string}){const theme=useConsumerTheme();return <View style={[s.resultStat,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[s.resultValue,{color:theme.accent}]}>{value}</Text><Text style={[s.resultLabel,{color:theme.muted}]}>{label}</Text></View>}
...
const s=StyleSheet.create({
 ...
 routeGrid:{gap:7},routeCard:{backgroundColor:'#f4f7f5',borderRadius:13,padding:11,borderWidth:1},routeName:{fontWeight:'900',color:'#173d2b'},routeMeta:{fontSize:10,color:'#64756b',marginTop:2},

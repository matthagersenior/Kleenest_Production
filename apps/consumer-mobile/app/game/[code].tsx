...
export default function GameArena(){
 ...
 return <SafeAreaView style={[s.safe,{backgroundColor:ui.canvas}]}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
  <View style={[s.hero,{backgroundColor:arena.surface}]}>
   <View style={s.topRow}><Pressable onPress={()=>router.back()} style={s.back}><Text style={s.backText}>‹ ARCADE</Text></Pressable><Text style={[s.arenaLabel,{color:arena.accent}]}>{arena.label}</Text></View>
  ...
  <View style={[s.challengeCard,{backgroundColor:ui.surface,borderColor:ui.line}]}><Text style={[s.masteryKicker,{color:ui.muted}]}>CHALLENGE THIS GAME</Text><Text style={[s.challengeTitle,{color:ui.ink}]}>Put your run against someone you follow.</Text><Text style={[s.challengeBody,{color:ui.muted}]}>They get the same game and score ceiling. Fresh scenarios can differ, so the match rewards mastery rather than memorizing one fixed question order.</Text>{challengeMessage?<Text style={[s.challengeNotice,{color:ui.accent}]}>{challengeMessage}</Text>:null}<View style={s.targetList}>{targets.slice(0,6).map(target=><View key={String(target.user_id)} style={[s.targetRow,{borderTopColor:ui.line}]}><Pressable style={{flex:1}} onPress={()=>router.push({pathname:'/contributor/[id]',params:{id:String(target.user_id)}})}><Text style={[s.targetName,{color:ui.ink}]}>{target.display_name||target.username||'Contributor'}</Text><Text style={[s.targetMeta,{color:ui.muted}]}>{target.relationship||'community'} · view profile →</Text></Pressable><Pressable style={[s.secondary,{backgroundColor:ui.surfaceRaised,borderColor:ui.line}]} onPress={async()=>{try{await createGameChallenge(game.code,String(target.user_id));setChallengeMessage('Challenge sent to '+(target.display_name||target.username||'player')+'.')}catch(error:any){setChallengeMessage(error?.message||'Challenge could not be sent.')}}}><Text style={[s.secondaryText,{color:ui.accent}]}>CHALLENGE</Text></Pressable></View>)}</View>{!targets.length?<Text style={s.challengeBody}>Follow contributors in Community to unlock rivals.</Text>:null}</View>
 ...
 function Builder(){
  ...
  return <><Text style={s.context}>BUILD LAB · SELECT EVERY SUPPORTED AMENITY</Text><Text style={[s.question,{color:ui.ink}]}>{builder.prompt}</Text><View style={s.answers}>{builder.options.map((option:string,index:number)=><Pressable key={option} onPress={()=>toggleBuilder(index)} style={[s.answer,{backgroundColor:ui.surfaceRaised,borderColor:ui.line},builderSelected.includes(index)&&{backgroundColor:arena.surface,borderColor:arena.surface}]}><Text style={[s.answerText,{color:ui.ink},builderSelected.includes(index)&&{color:'#fff'}]}>{builderSelected.includes(index)?'✓ ':''}{option}</Text></Pressable>)}</View><Pressable style={[s.primary,{backgroundColor:ui.accent}]} onPress={submitBuilder}><Text style={[s.primaryText,{color:ui.accentText}]}>LOCK PROFILE</Text></Pressable></>;
 }
 function Memory(){
  return <><Text style={s.context}>MEMORY GRID · {memoryMoves} MOVES</Text><Text style={[s.question,{color:ui.ink}]}>Match each restroom condition to the evidence that proves it.</Text><View style={s.memoryGrid}>{memoryCards.map((card,index)=>{const shown=memoryOpen.includes(index)||memoryMatched.includes(index);return <Pressable key={index} onPress={()=>flipMemory(index)} style={[s.memoryCard,{backgroundColor:ui.surfaceRaised,borderColor:ui.line},memoryMatched.includes(index)&&{backgroundColor:arena.surface,borderColor:arena.surface}]}><Text style={[s.memoryText,{color:ui.ink},memoryMatched.includes(index)&&{color:'#fff'}]}>{shown?card.text:'?'}</Text></Pressable>})}</View></>;
 }
 function Results(){
  return <View style={s.results}><Text style={[s.resultsKicker,{color:ui.muted}]}>{lives<=0?'RUN ENDED':'RUN COMPLETE'}</Text><Text style={[s.resultsScore,{color:ui.ink}]}>{score}</Text><Text style={[s.resultsLabel,{color:ui.muted}]}>FINAL SCORE</Text><View style={s.resultStats}><Result value={accuracy+'%'} label="accuracy"/><Result value={maxCombo+'×'} label="best combo"/><Result value={mastery} label="mastery"/></View><Pressable disabled={saved} style={[s.primary,{backgroundColor:ui.accent},saved&&s.disabled]} onPress={save}><Text style={[s.primaryText,{color:ui.accentText}]}>{saved?'RUN SAVED':'SAVE RUN'}</Text></Pressable><View style={s.resultActions}><Pressable style={[s.secondary,{backgroundColor:ui.surfaceRaised,borderColor:ui.line}]} onPress={reset}><Text style={[s.secondaryText,{color:ui.accent}]}>PLAY AGAIN</Text></Pressable><Pressable style={[s.secondary,{backgroundColor:ui.surfaceRaised,borderColor:ui.line}]} onPress={()=>router.push('/games')}><Text style={[s.secondaryText,{color:ui.accent}]}>ARCADE</Text></Pressable><Pressable style={[s.secondary,{backgroundColor:ui.surfaceRaised,borderColor:ui.line}]} onPress={()=>router.push('/progress')}><Text style={[s.secondaryText,{color:ui.accent}]}>LEAGUE</Text></Pressable></View></View>;
 }
}

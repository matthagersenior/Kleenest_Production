...
export default function GameArena(){
 ...
 return <SafeAreaView style={[s.safe,{backgroundColor:theme.soft}]}><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
  ...
  <View style={s.challengeCard}><Text style={s.masteryKicker}>CHALLENGE THIS GAME</Text><Text style={s.challengeTitle}>Put your run against someone you follow.</Text><Text style={s.challengeBody}>They get the same game and score ceiling. Fresh scenarios can differ, so the match rewards mastery rather than memorizing one fixed question order.</Text>{challengeMessage?<Text style={s.challengeNotice}>{challengeMessage}</Text>:null}<View style={s.targetList}>{targets.slice(0,6).map(target=><View key={String(target.user_id)} style={s.targetRow}><Pressable style={{flex:1}} onPress={()=>router.push({pathname:'/contributor/[id]',params:{id:String(target.user_id)}})}><Text style={s.targetName}>{target.display_name||target.username||'Contributor'}</Text><Text style={s.targetMeta}>{target.relationship||'community'} · view profile →</Text></Pressable><Pressable style={[s.secondary,{backgroundColor:theme.soft}]} onPress={async()=>{try{await createGameChallenge(game.code,String(target.user_id));setChallengeMessage('Challenge sent to '+(target.display_name||target.username||'player')+'.')}catch(error:any){setChallengeMessage(error?.message||'Challenge could not be sent.')}}}><Text style={s.secondaryText}>CHALLENGE</Text></Pressable></View>)}</View>{!targets.length?<Text style={s.challengeBody}>Follow contributors in Community to unlock rivals.</Text>:null}</View>

...
export default function HomeScreen(){
  ...
  useEffect(()=>{let active=true;const client=getKleenestSupabaseClient();async function check(){const{data}=await client.auth.getSession();if(!active)return;if(!data.session){setPolicyRequired(false);return}setPolicyRequired(!(await hasCurrentPolicyAcceptance().catch(()=>true)))}void check();const auth=client.auth.onAuthStateChange(()=>{void check()});return()=>{active=false;auth.data.subscription.unsubscribe()}},[]);

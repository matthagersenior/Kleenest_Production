...
export default function LocationQrScreen(){
  ...
  return <SafeAreaView style={[s.safe,{backgroundColor:theme.canvas}]}><ScrollView contentContainerStyle={s.page}>
    ...
    {identity?<View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}>
      <Text style={[s.kicker,{color:theme.muted}]}>WHAT THIS QR WILL SHOW</Text>
      {identity.brand_name?<Text style={[s.cardTitle,{color:theme.ink}]}>{identity.brand_name}</Text>:null}
      <Text style={[s.meta,{color:theme.muted}]}>{[identity.location_address,identity.location_city,identity.location_state].filter(Boolean).join(', ')||'Kleenest location details'}</Text>
      {identity.rating!=null?<Text style={[s.meta,{color:theme.ink}]}>★ {Number(identity.rating).toFixed(1)}{identity.review_count!=null?' · '+Number(identity.review_count)+' reviews':''}</Text>:null}
      {identity.business_claimed&&identity.business_name?<View style={[s.businessInfo,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.kicker,{color:theme.accent}]}>VERIFIED BUSINESS CONTEXT</Text><Text style={[s.businessName,{color:theme.ink}]}>{identity.business_name}</Text>{identity.business_description?<Text style={[s.meta,{color:theme.muted}]}>{identity.business_description}</Text>:null}</View>:<Text style={[s.meta,{color:theme.muted}]}>Unclaimed: the QR shows Kleenest community/location information and keeps business authority separate.</Text>}
      <Text style={[s.meta,{color:theme.muted}]}>Every location QR can open the same Kleenest location details, feedback, verified check-in and review workflow.</Text>
    </View>:null}
    ...
    <View style={[s.qrCard,{backgroundColor:theme.surface,borderColor:theme.line}]}>
      ...
      <Text style={[s.meta,{color:theme.muted}]}>Scan → location info → feedback, or verify an on-site visit → review. Claimed/integrated Business context is added to this same permanent identity.</Text>

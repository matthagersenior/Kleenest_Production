...
export async function refreshConsumerLiveNetworkRegions(){
  ...
  await Location.startGeofencingAsync(CONSUMER_LIVE_NETWORK_TASK,nearby.map(row=>({identifier:row.id,latitude:row.latitude,longitude:row.longitude,radius:200,notifyOnEnter:true,notifyOnExit:true})));

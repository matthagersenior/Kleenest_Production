...
const emptySummary=(days:number):WeekInReviewSummary=>({periodDays:days,startedAt:null,visitCount:0,placeCount:0,verifiedVisitCount:0,reviewedCount:0,reviewReadyCount:0,verificationAvailableCount:0,photoOpenCount:0,visits:[]});
...
export async function getWeekInReview(days=7):Promise<WeekInReviewSummary>{
  ...
  const visits=(Array.isArray(raw.visits)?raw.visits:[]).map((item:any):WeekInReviewVisit=>({
    ...
    checkInId:stringOrNull(item.check_in_id),checkedInAt:stringOrNull(item.checked_in_at),verificationMethod:stringOrNull(item.verification_method),reviewId:stringOrNull(item.review_id),reviewedAt:stringOrNull(item.reviewed_at),reviewPhotoCount:Number(item.review_photo_count||0),photoOpen:item.photo_open===true,
  ...
  return{
    ...
    reviewedCount:Number(raw.reviewed_count||0),reviewReadyCount:Number(raw.review_ready_count||0),verificationAvailableCount:Number(raw.verification_available_count||0),photoOpenCount:Number(raw.photo_open_count||0),visits,
  };
}

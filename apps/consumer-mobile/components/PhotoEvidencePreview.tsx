...
export default function PhotoEvidencePreview({photos,maxCount=3,size=96}:{photos:any[];maxCount?:number;size?:number}){
  ...
  return <View style={s.row}>{visible.map(photo=><Image key={String(photo.storage_path||photo.public_url)} source={{uri:String(photo.public_url)}} accessibilityLabel="Restroom review evidence photo" style={[s.photo,{width:size,height:size,backgroundColor:theme.surfaceRaised}]}/>)}</View>;
}

...
export function placeSignal(item:Row):Signal{const name=String(item?.name||'');for(const [pattern,signal] of brandSignals)if(pattern.test(name))return signal;return placeSignals[String(item?.place_type||'').toLowerCase()]||{icon:'🚻',label:'Restroom location'};}

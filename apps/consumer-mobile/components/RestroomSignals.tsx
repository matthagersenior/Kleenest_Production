...
function inferredBusinessName(item:Row){const explicit=String(item?.business_name||item?.business?.name||item?.brand||item?.operator_name||'').trim();if(explicit)return explicit;const name=String(item?.name||'').trim();const generic=/^(unnamed|public restroom|restroom|bathroom|toilet|park|playground|trail|station)\b/i.test(name);return name&&!generic?name:'';}
...
export function PlaceIcon({item,size=24}:{item:Row;size?:number}){const logo=String(item?.business_logo_url||item?.business?.logo_url||'').trim(),businessName=inferredBusinessName(item);if(logo)return <Image accessibilityLabel={`${businessName||item?.name||'Business'} logo`} source={{uri:logo}} resizeMode="contain" style={{width:size,height:size,borderRadius:Math.max(4,size*.2)}}/>;if(businessName)return <View accessibilityLabel={`${businessName} business icon`} style={{width:size,height:size,borderRadius:Math.max(6,size*.24),backgroundColor:palette.green,borderWidth:1,borderColor:'#fff',alignItems:'center',justifyContent:'center'}}><Text adjustsFontSizeToFit numberOfLines={1} style={{color:'#fff',fontWeight:'900',letterSpacing:-.4,fontSize:Math.max(8,size*.38)}}>{initials(businessName)}</Text></View>;return <Text style={{fontSize:size*.82}}>{restroomMarkerGlyph(item)}</Text>}

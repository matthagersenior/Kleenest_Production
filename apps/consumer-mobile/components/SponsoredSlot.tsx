...
export function SponsoredSlot({surface,context={},contextClass}:{surface:string;context?:Record<string,unknown>;contextClass?:string}){
  ...
  if(!card)return null;
  async function open(){void recordSponsoredEvent(card,'click',contextClass||surface);await Linking.openURL(card.destination_url)}
  function dismiss(){void recordSponsoredEvent(card,'dismiss',contextClass||surface);setCard(null)}
  return <View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}>
    <View style={s.top}><Text style={[s.label,{color:theme.muted}]}>{card.label.toUpperCase()} · {card.sponsor_name}</Text><Pressable accessibilityLabel="Hide sponsored card" onPress={dismiss}><Text style={[s.close,{color:theme.muted}]}>×</Text></Pressable></View>
    <Text style={[s.title,{color:theme.ink}]}>{card.headline}</Text>
    {card.body?<Text style={[s.body,{color:theme.muted}]}>{card.body}</Text>:null}
    <Pressable style={[s.cta,{backgroundColor:theme.accentSoft,borderColor:theme.line}]} onPress={open}><Text style={[s.ctaText,{color:theme.accent}]}>{card.cta_label} →</Text></Pressable>

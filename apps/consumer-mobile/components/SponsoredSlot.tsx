...
export function SponsoredSlot({surface,context={},contextClass,fallback=null}:{surface:string;context?:Record<string,unknown>;contextClass?:string;fallback?:ReactNode}){
  ...
  return <View style={[s.card,{backgroundColor:theme.surface,borderColor:theme.line}]}>
    ...
    {card.image_url&&card.creative_mode!=='text_only'?<Image source={{uri:card.image_url}} accessibilityLabel={card.image_alt||`${card.sponsor_name} sponsored image`} resizeMode="cover" style={s.image}/>:null}
    {card.creative_mode!=='image_only'?<Text style={[s.title,{color:theme.ink}]}>{card.headline}</Text>:null}
    {card.creative_mode!=='image_only'&&card.body?<Text style={[s.body,{color:theme.muted}]}>{card.body}</Text>:null}

...
import {
  advanceFleetRealWorldDemoLoop,
  getFleetRealWorldDemoLoopState,
  resetFleetRealWorldDemoLoop,
  startFleetRealWorldDemoLoop,
...
export default function FleetDemo(){
 ...
 const[message,setMessage]=useState('Loading guided Fleet loop…');
 ...
 async function load(){
  ...
  try{
   ...
   setBusinessId(id);
   setData(await getFleetRealWorldDemoLoopState(id));
   setMessage('');
  }catch(e:any){
   setData(null);
   setMessage(e?.message||'Select Kleenest Demo Fleet to run this loop.');
 ...
 async function act(kind:'start'|'advance'|'reset'){
  ...
  try{
   const next=kind==='start'
    ?await startFleetRealWorldDemoLoop(businessId)
    :kind==='advance'
      ?await advanceFleetRealWorldDemoLoop(businessId)
      :await resetFleetRealWorldDemoLoop(businessId);
   setData(next);
   setMessage(kind==='start'?'Fleet demo started. Follow the current operating step, inspect the live control if useful, then complete the step.':kind==='reset'?'Fleet demo loop reset.':'Step completed and Fleet evidence captured.');
  }catch(e:any){
   setMessage(e?.message||'Fleet demo action failed.');
  }finally{setBusy(false);}
 }
 ...
 >
  ...
  {!data?<View style={s.card}>
   <Text style={s.cardTitle}>Choose Kleenest Demo Fleet</Text>
   <Text style={s.meta}>The guided Fleet loop is restricted to the isolated demo workspace.</Text>
   <Link href="/workspaces" style={s.linkButton}>Open Workspaces →</Link>
  </View>:<>
   <View style={s.statusCard}>
    <View style={{flex:1,minWidth:0}}>
     <Text style={s.eyebrow}>FLEET LOOP · {String(data.workspace||'Kleenest Demo Fleet')}</Text>
   ...
   {!data.started?<View style={s.startCard}>
    ...
    <Text style={s.startTitle}>Start with an assigned mission and follow it through the field.</Text>
    <Text style={s.meta}>Each step explains the operational state change, why dispatch cares, and which Fleet evidence proves the step. Real Fleet screens are secondary inspection points instead of the entire demo.</Text>
   ...
   {data.completed?<View style={s.completeCard}>
    ...
    <Text style={s.completeBody}>The demo preserved the whole chain: assignment, route context, dispatch, stop proof, exception recovery and the evidence that improves the next mission.</Text>
   ...
   <View style={s.timelineHeader}>
    ...
    <Text style={s.meta}>Progress persists when you leave the demo to inspect Planner, Dispatch, Execution, Operations or Insights.</Text>
   </View>
    ...
    {steps.map((step,index)=>{
     ...
     return <View key={step.id} style={[s.timelineStep,done&&s.timelineDone,active&&s.timelineActive]}>
      ...
      <View style={{flex:1,minWidth:0,gap:3}}>
       ...
       {event?<Text style={s.eventText}>Fleet evidence captured when this step completed.</Text>:null}
      </View>
     </View>;
    })}
   </View>
...
const s=StyleSheet.create({
 ...
 completeBody:{fontSize:12,lineHeight:18,color:'#d8e7df'},

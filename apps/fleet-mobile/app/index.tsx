...
export default function FleetHome(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.resolved==='dark'?theme.surfaceRaised:'#173f2d',borderColor:theme.resolved==='dark'?theme.line:'#173f2d',borderWidth:1}]}><Text style={s.eyebrow}>KLEENEST FLEET · HUMAN-INFRASTRUCTURE INTELLIGENCE</Text><Text style={s.title}>{workspace?.business_name||workspace?.name||'Mobile workforce intelligence'}</Text><Text style={s.copy}>{FLEET_POSITIONING.promise}</Text></View>

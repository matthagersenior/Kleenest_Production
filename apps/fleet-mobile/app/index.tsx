...
import { getFleetOnboardingState } from '../services/onboarding';
...
export default function FleetHome(){
 const[data,setData]=useState<any>(null),[workspace,setWorkspace]=useState<any>(null),[onboarding,setOnboarding]=useState<any>({}),[busy,setBusy]=useState(false),[message,setMessage]=useState('Loading Fleet command center…');
 async function load(){setBusy(true);try{const id=await currentFleetBusinessId();const spaces=await listFleetWorkspaceOptions();setWorkspace(spaces.find((r:any)=>String(r.business_id)===id)||spaces[0]||null);const[dashboard,state]=await Promise.all([getFleetDashboard(id),getFleetOnboardingState(id).catch(()=>({}))]);setData(dashboard);setOnboarding(state);setMessage('')}catch(e:any){setMessage(e?.message||'Fleet workspace unavailable.')}finally{setBusy(false)}}
 useEffect(()=>{void load()},[]);
 ...
 const headline=String(onboarding?.experience?.headline||'Plan, dispatch, execute, monitor, recover offline work and measure results from one operational control plane.');

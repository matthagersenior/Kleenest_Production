...
export default function FleetHome(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.resolved==='dark'?theme.surfaceRaised:'#173f2d',borderColor:theme.resolved==='dark'?theme.line:'#173f2d',borderWidth:1}]}><Text style={s.eyebrow}>KLEENEST FLEET</Text><Text style={s.title}>{workspace?.business_name||workspace?.name||'Fleet mission control'}</Text><Text style={s.copy}>{headline}</Text></View>
  ...
  {priority.length?<View style={s.priorityWrap}><Text style={s.eyebrowDark}>YOUR PRIORITIES</Text><Text style={[s.sectionTitle,{color:theme.ink}]}>Targeted from onboarding</Text><Text style={[s.meta,{color:theme.muted}]}>Fleet is putting the workflows tied to your goals, pain points and team ownership first.</Text>{priority.map(([href,title,body])=><Link key={href} href={href as any} asChild><Pressable accessibilityRole="button" style={[s.card,s.priorityCard]}><View style={{flex:1,gap:4}}><Text style={[s.priorityLabel,{color:theme.accent}]}>PRIORITY</Text><Text style={[s.cardTitle,{color:theme.ink}]}>{title}</Text><Text style={[s.meta,{color:theme.muted}]}>{body}</Text></View><Text style={s.arrow}>›</Text></Pressable></Link>)}</View>:null}
  ...
  <View style={s.links}><Link href="/notifications" style={[s.link,{backgroundColor:theme.surface,borderColor:theme.line}]}>Notifications</Link><Link href="/account" style={[s.link,{backgroundColor:theme.surface,borderColor:theme.line}]}>Account</Link><Link href="/support" style={[s.link,{backgroundColor:theme.surface,borderColor:theme.line}]}>Support</Link><Link href="/terms" style={[s.link,{backgroundColor:theme.surface,borderColor:theme.line}]}>Terms</Link><Link href="/privacy" style={[s.link,{backgroundColor:theme.surface,borderColor:theme.line}]}>Privacy</Link></View>
 </ScrollView>
}

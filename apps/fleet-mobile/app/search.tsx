...
export default function GlobalSearch(){
 ...
 return <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.surface,borderColor:theme.line}]}>
   <Text style={[s.eyebrow,{color:theme.accent}]}>SEARCH FLEET</Text><Text style={[s.title,{color:theme.ink}]}>Find anything in Fleet.</Text>
   <Text style={[s.body,{color:theme.muted}]}>{operator?'Search routes, drivers, vehicles, alerts, controls and how Fleet features work.':'Search the Fleet features available to your role, including nearby, alerts, account and support.'}</Text>
   <TextInput autoFocus value={query} onChangeText={setQuery} onSubmitEditing={()=>void remember()} placeholder={operator?'Try “75 seats”, a driver, vehicle, route, or geofencing…':'Try “nearby”, “alerts”, or “offline”…'} placeholderTextColor={theme.muted} style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/>
  </View>

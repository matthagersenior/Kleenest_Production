...
export default function FleetMemberWorkspace(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.resolved==='dark'?theme.surfaceRaised:'#315f8c',borderColor:theme.resolved==='dark'?theme.line:'#315f8c',borderWidth:1}]}><Text style={s.kicker}>MY KLEENEST · MOBILE WORKFORCE</Text><Text style={s.title}>{workspace.business_name||'Fleet member workspace'}</Text><Text style={s.body}>{role==='operator'?FLEET_POSITIONING.operator:role==='driver'?'Your assigned route stays connected to Kleenest Route Relief, trusted-stop discovery, geofencing and field alerts.':'Your organization gives you Kleenest worker utility and benefits without exposing operator controls.'}</Text><View style={s.badges}><Badge label={role.toUpperCase()}/><Badge label={premium?'PREMIUM ACTIVE':'FLEET MEMBER'}/>{canDrive?<Badge label="ROUTE CONNECTED"/>:null}</View></View>
  ...
  <View style={s.links}><Link href="/nearby" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}>Nearby</Link><Link href="/notifications" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}>Notifications</Link><Link href="/workspaces" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}>Workspaces</Link><Link href="/account" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}>Account</Link></View>
 </ScrollView>
}

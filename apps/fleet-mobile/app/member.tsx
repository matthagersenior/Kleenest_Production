...
import { FLEET_POSITIONING } from '../services/positioning';
...
export default function FleetMemberWorkspace(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,borderWidth:1}]}><Text style={[s.kicker,{color:theme.accent}]}>MY KLEENEST · MOBILE WORKFORCE</Text><Text style={[s.title,{color:theme.ink}]}>{workspace.business_name||'Fleet member workspace'}</Text><Text style={[s.body,{color:theme.muted}]}>{role==='operator'?FLEET_POSITIONING.operator:role==='driver'?'Your assigned route stays connected to Kleenest Route Relief, trusted-stop discovery, geofencing and field alerts.':'Your organization gives you Kleenest worker utility and benefits without exposing operator controls.'}</Text><View style={s.badges}><Badge label={role.toUpperCase()}/><Badge label={premium?'PREMIUM ACTIVE':'FLEET MEMBER'}/>{canDrive?<Badge label="ROUTE CONNECTED"/>:null}</View></View>
  {message?<Text accessibilityLiveRegion="polite" style={[s.message,{color:theme.muted}]}>{message}</Text>:null}

  <View style={[s.consumerCard,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.eyebrow,{color:theme.accent}]}>ROUTE RELIEF + WORKFORCE ACCESS</Text><Text style={[s.cardTitle,{color:theme.ink}]}>The workday should not make a basic restroom stop difficult.</Text><Text style={[s.meta,{color:theme.muted}]}>{FLEET_POSITIONING.member} Search here or ahead, inspect Kleenest trust/access signals, navigate, and use organization-provided Premium when active.</Text><View style={s.actionRow}><Link href="/nearby" asChild><Pressable style={[s.action,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Open Route Relief</Text></Pressable></Link><Pressable onPress={()=>void openConsumer()} style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Open full Kleenest</Text></Pressable></View></View>

  {canDrive?<><View style={s.sectionHead}><Text style={[s.eyebrow,{color:theme.accent}]}>MY ROUTE</Text><Text style={[s.sectionTitle,{color:theme.ink}]}>{route?String(route.name||'Assigned route'):'No active assignment'}</Text><Text style={[s.meta,{color:theme.muted}]}>{route?String(route.status||'planned').replaceAll('_',' ')+' · '+stops.length+' stops':'When dispatch assigns you a route, it appears here automatically.'}</Text></View>
   {route?<View style={[s.routeCard,{backgroundColor:theme.surface,borderColor:theme.line}]}><View style={s.metrics}><Metric label="STOP" value={nextStop?String(nextStop.stop_order||'—'):'✓'}/><Metric label="STATUS" value={nextStop?String(nextStop.status||'planned').toUpperCase():'DONE'}/><Metric label="VEHICLE" value={String(dispatch?.vehicle?.unit_code||dispatch?.vehicle?.name||'—')}/></View>{nextStop?<><Text style={[s.nextTitle,{color:theme.ink}]}>{String(nextStop.stop_name||'Next route stop')}</Text><Text style={[s.meta,{color:theme.muted}]}>{String(nextStop.stop_address||'Address unavailable')}</Text>{dwellLabel(nextStop,policy)?<View style={[dwellLabel(nextStop,policy)?.startsWith('STALL')?s.stall:s.dwell,{backgroundColor:dwellLabel(nextStop,policy)?.startsWith('STALL')?theme.surfaceRaised:theme.accentSoft,borderColor:theme.line,borderWidth:1}]}><Text style={[s.alertText,{color:theme.ink}]}>{dwellLabel(nextStop,policy)}</Text></View>:null}<View style={s.actionRow}><Pressable onPress={()=>void navigate(nextStop)} style={[s.action,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Start navigation</Text></Pressable><Pressable onPress={()=>void enableLive()} disabled={busy} style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Enable geofencing</Text></Pressable></View></>:<Text style={[s.done,{color:theme.muted}]}>All route stops are terminal. Dispatch will update the route as completion is reconciled.</Text>}</View>:null}
  ...
  </>:<View style={[s.infoCard,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.eyebrow,{color:theme.accent}]}>FLEET GATE</Text><Text style={[s.cardTitle,{color:theme.ink}]}>Operator controls are intentionally hidden.</Text><Text style={[s.meta,{color:theme.muted}]}>Planning, dispatch, assets, enterprise operations and organization analytics stay available only to Fleet operators. If you are assigned as a driver, route execution and geofencing unlock automatically without exposing manager controls.</Text></View>}

  <View style={s.links}><Link href="/nearby" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.accent}]}>Nearby</Link><Link href="/notifications" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.accent}]}>Notifications</Link><Link href="/workspaces" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.accent}]}>Workspaces</Link><Link href="/account" style={[s.link,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.accent}]}>Account</Link></View>
 </ScrollView>
}
...
function StopCard({stop,policy,busy,onMark,onNavigate}:{stop:Row;policy:Row;busy:boolean;onMark:(event:'arrived'|'service_started'|'completed'|'departed'|'skipped')=>void|Promise<void>;onNavigate:()=>void|Promise<void>}){
 ...
 return <View style={[s.stop,{backgroundColor:theme.surface,borderColor:theme.line}]}><View style={s.stopTop}><View style={[s.stopNumber,{backgroundColor:theme.accentSoft}]}><Text style={[s.stopNumberText,{color:theme.accent}]}>{String(stop.stop_order||'—')}</Text></View><View style={{flex:1,gap:2}}><Text style={[s.stopTitle,{color:theme.ink}]}>{String(stop.stop_name||'Route stop')}</Text><Text style={[s.meta,{color:theme.muted}]}>{String(stop.stop_address||'Address unavailable')}</Text></View><Badge label={status.toUpperCase()}/></View>{dwell?<View style={[dwell.startsWith('STALL')?s.stall:s.dwell,{backgroundColor:dwell.startsWith('STALL')?theme.surfaceRaised:theme.accentSoft,borderColor:theme.line,borderWidth:1}]}><Text style={[s.alertText,{color:theme.ink}]}>{dwell}</Text></View>:null}<View style={s.actionRow}><Pressable onPress={onNavigate} style={[s.smallSecondary,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Navigate</Text></Pressable>{status==='planned'?<Pressable disabled={busy} onPress={()=>onMark('arrived')} style={[s.smallAction,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Arrived</Text></Pressable>:null}{status==='arrived'?<Pressable disabled={busy} onPress={()=>onMark('service_started')} style={[s.smallAction,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Start service</Text></Pressable>:null}{['arrived','servicing'].includes(status)?<Pressable disabled={busy} onPress={()=>onMark('completed')} style={[s.smallAction,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Complete</Text></Pressable>:null}{!['completed','skipped','cancelled'].includes(status)?<Pressable disabled={busy} onPress={()=>onMark('skipped')} style={[s.smallSecondary,{backgroundColor:theme.surfaceRaised,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Skip</Text></Pressable>:null}</View></View>
}
...
function Badge({label}:{label:string}){const theme=useFleetTheme();return <View style={[s.badge,{backgroundColor:theme.accentSoft}]}><Text style={[s.badgeText,{color:theme.accent}]}>{label}</Text></View>}
function Metric({label,value}:{label:string;value:string}){const theme=useFleetTheme();return <View style={[s.metric,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text numberOfLines={1} style={[s.metricValue,{color:theme.ink}]}>{value}</Text><Text style={[s.metricLabel,{color:theme.muted}]}>{label}</Text></View>}

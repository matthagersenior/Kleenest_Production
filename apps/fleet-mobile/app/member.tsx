...
export default function FleetMemberWorkspace(){
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={load}/>} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.resolved==='dark'?theme.surfaceRaised:'#315f8c',borderColor:theme.resolved==='dark'?theme.line:'#315f8c',borderWidth:1}]}><Text style={s.kicker}>MY KLEENEST · FLEET</Text><Text style={s.title}>{workspace.business_name||'Fleet member workspace'}</Text><Text style={s.body}>{role==='operator'?'Your operator account keeps the full Fleet control plane, plus this personal Kleenest experience.':role==='driver'?'Your assigned routes, geofences and stop events live beside your personal Kleenest discovery experience.':'Your organization-connected Kleenest experience keeps consumer discovery and benefits separate from operator controls.'}</Text><View style={s.badges}><Badge label={role.toUpperCase()}/><Badge label={premium?'PREMIUM ACTIVE':'FLEET MEMBER'}/>{canDrive?<Badge label="ROUTE EXECUTION"/>:null}</View></View>
  {message?<Text accessibilityLiveRegion="polite" style={[s.message,{color:theme.muted}]}>{message}</Text>:null}

  <View style={[s.consumerCard,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.eyebrow,{color:theme.accent}]}>YOUR CONSUMER EXPERIENCE</Text><Text style={[s.cardTitle,{color:theme.ink}]}>Kleenest stays useful even when you are off route.</Text><Text style={[s.meta,{color:theme.muted}]}>Search any area, see consumer-facing location photos, cleanliness and access signals, start navigation, receive notifications and use your Fleet-provided Premium entitlement when active.</Text><View style={s.actionRow}><Link href="/nearby" asChild><Pressable style={[s.action,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Find nearby bathrooms</Text></Pressable></Link><Pressable onPress={()=>void openConsumer()} style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Open full Kleenest</Text></Pressable></View></View>

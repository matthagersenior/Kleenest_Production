...
export default function FleetNearby(){
 ...
 const visibleRows=useMemo(()=>smartOnly?rows.filter(row=>row.smart_bathroom===true):rows,[rows,smartOnly]);
 const chosen=useMemo(()=>visibleRows.find(row=>routeLocationId(row)===selected)||null,[visibleRows,selected]);
 ...
 return <ScrollView refreshControl={<RefreshControl refreshing={busy} onRefresh={()=>center?loadAt(center):useMyLocation()}/>} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={[s.page,{backgroundColor:theme.canvas}]}>
  <View style={[s.hero,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,borderWidth:1}]}><Text style={[s.kicker,{color:theme.accent}]}>ROUTE RELIEF · KLEENEST FLEET</Text><Text style={[s.title,{color:theme.ink}]}>Trusted stops for people working on the move.</Text><Text style={[s.body,{color:theme.muted}]}>{workspace?.business_name?'Connected to '+workspace.business_name+'. ':''}{FLEET_POSITIONING.routeRelief} Fleet keeps this worker utility connected to the same Kleenest network without trying to replace the organization’s dispatch system.</Text></View>
  <View style={[s.searchCard,{backgroundColor:theme.surface,borderColor:theme.line}]}><Text style={[s.label,{color:theme.muted}]}>SEARCH HERE OR AHEAD</Text><View style={s.searchRow}><TextInput value={query} onChangeText={setQuery} onSubmitEditing={()=>void searchArea()} placeholder="Home, work, school, address, city…" placeholderTextColor={theme.muted} style={[s.input,{backgroundColor:theme.surfaceRaised,borderColor:theme.line,color:theme.ink}]}/><Pressable onPress={()=>void searchArea()} disabled={busy} style={[s.action,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Search</Text></Pressable></View><Pressable onPress={()=>void useMyLocation()} disabled={busy} style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>⌖ Use my location</Text></Pressable><View style={s.chips}>{RADII.map(value=><Pressable key={value} onPress={()=>setRadiusMiles(value)} style={[s.chip,{backgroundColor:value===radiusMiles?theme.accent:theme.surfaceRaised,borderColor:value===radiusMiles?theme.accent:theme.line,borderWidth:1}]}><Text style={[s.chipText,{color:value===radiusMiles?theme.accentText:theme.ink}]}>{value} mi</Text></Pressable>)}<Pressable onPress={()=>setSmartOnly(v=>!v)} style={[s.chip,{backgroundColor:smartOnly?theme.accent:theme.surfaceRaised,borderColor:smartOnly?theme.accent:theme.line,borderWidth:1}]}><Text style={[s.chipText,{color:smartOnly?theme.accentText:theme.ink}]}>⌁ Smart only</Text></Pressable></View></View>
  {message?<Text accessibilityLiveRegion="polite" style={[s.message,{color:theme.muted}]}>{message}</Text>:null}
  {center?<FleetMap center={centerTuple} locations={visibleRows} selectedId={selected} mode="nearby" onSelect={row=>setSelected(routeLocationId(row))}/>:null}
  {chosen?<View style={[s.selectedCard,{backgroundColor:theme.surface,borderColor:theme.line}]}>{chosen.consumer_photo_url?<View><Image source={{uri:String(chosen.consumer_photo_url)}} style={s.heroPhoto}/><View style={s.photoBadge}><Text style={s.photoBadgeText}>LOCATION PHOTO</Text></View></View>:null}<Text style={[s.cardTitle,{color:theme.ink}]}>{String(chosen.name||chosen.business_name||'Kleenest location')}</Text><Text style={[s.meta,{color:theme.muted}]}>{[chosen.address,chosen.city,chosen.state].filter(Boolean).join(', ')||'Address unavailable'}</Text><View style={s.factRow}>{chosen.rating!=null?<Fact label="RATING" value={Number(chosen.rating).toFixed(1)+' ★'}/>:null}<Fact label="DISTANCE" value={distance(chosen.distance_meters)}/><Fact label="ACCESS" value={chosen.accessible?'YES':'—'}/>{chosen.smart_bathroom?<Fact label="SMART" value="⌁ YES"/>:null}</View><View style={s.actions}><Pressable onPress={()=>void navigate(chosen)} style={[s.action,{backgroundColor:theme.accent}]}><Text style={[s.actionText,{color:theme.accentText}]}>Start navigation</Text></Pressable><Pressable onPress={()=>void openConsumer()} style={[s.secondary,{backgroundColor:theme.accentSoft,borderColor:theme.line}]}><Text style={[s.secondaryText,{color:theme.accent}]}>Open full Kleenest</Text></Pressable></View></View>:null}
  <View style={s.sectionHead}><Text style={[s.sectionTitle,{color:theme.ink}]}>Trusted options</Text><Text style={[s.meta,{color:theme.muted}]}>{visibleRows.length} locations · {radiusMiles} mile radius{smartOnly?' · Smart Restroom only':''}</Text></View>
  {visibleRows.slice(0,30).map(row=><Pressable key={routeLocationId(row)} onPress={()=>setSelected(routeLocationId(row))} style={[s.result,{backgroundColor:theme.surface,borderColor:theme.line},selected===routeLocationId(row)&&{backgroundColor:theme.accentSoft,borderColor:theme.accent}]}>{row.consumer_photo_url?<Image source={{uri:String(row.consumer_photo_url)}} style={s.thumb}/>:<View style={[s.thumbFallback,{backgroundColor:theme.accentSoft}]}><Text style={[s.thumbLetter,{color:theme.accent}]}>{String(row.name||row.business_name||'K').slice(0,1).toUpperCase()}</Text></View>}<View style={{flex:1,gap:2}}><Text style={[s.resultTitle,{color:theme.ink}]}>{String(row.name||row.business_name||'Kleenest location')}</Text><Text style={[s.meta,{color:theme.muted}]} numberOfLines={2}>{[row.address,row.city,row.state].filter(Boolean).join(', ')||'Address unavailable'}</Text><Text style={[s.signal,{backgroundColor:theme.accentSoft,color:theme.accent}]}>{[row.rating!=null?Number(row.rating).toFixed(1)+' ★':null,row.bathroom_verification_status?String(row.bathroom_verification_status).replaceAll('_',' '):null,row.accessible?'Accessible':null,row.smart_bathroom?'⌁ Smart Restroom':null].filter(Boolean).join(' · ')||'Kleenest network location'}</Text></View><Text style={[s.distance,{color:theme.accent}]}>{distance(row.distance_meters)}</Text></Pressable>)}
 </ScrollView>
}
...
function Fact({label,value}:{label:string;value:string}){const theme=useFleetTheme();return <View style={[s.fact,{backgroundColor:theme.surfaceRaised}]}><Text style={[s.factValue,{color:theme.ink}]}>{value}</Text><Text style={[s.factLabel,{color:theme.muted}]}>{label}</Text></View>}

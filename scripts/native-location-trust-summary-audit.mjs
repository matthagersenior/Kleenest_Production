...
const failures=[];
for(const file of[migrationPath,servicePath,explorePath,adaptiveExplorePath,inventoryPath,formatterPath])if(!fs.existsSync(file))failures.push(`missing location trust file: ${file}`);
if(!failures.length){
 const migration=fs.readFileSync(migrationPath,'utf8'),service=fs.readFileSync(servicePath,'utf8'),exploreEntry=fs.readFileSync(explorePath,'utf8'),adaptiveExplore=fs.readFileSync(adaptiveExplorePath,'utf8'),inventory=fs.readFileSync(inventoryPath,'utf8'),formatter=fs.readFileSync(formatterPath,'utf8');
 ...
 if(!mapsTrustIds||!batchesTrust||!attachesTrust)failures.push('Explore must enrich the result set in one batched trust request.');

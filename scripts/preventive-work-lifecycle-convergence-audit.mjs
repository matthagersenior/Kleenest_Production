...
for(const token of ["params.get('work_order')",'focusPreventiveWorkOrderId','focusWorkOrderId={focusPreventiveWorkOrderId}'])need(businessControl,token,'Preventive alert deep-link control');
...
for(const token of ['Proof-backed completed','PHOTO PROOF','business photo proof','Due soon','Escalated','Critical overdue','CRITICAL OVERDUE','DUE SOON','scheduleSummary.critical_overdue','preventionUrl(row.id)','never substitute for visitor evidence'])need(fleet,token,'Fleet preventive lifecycle presentation');

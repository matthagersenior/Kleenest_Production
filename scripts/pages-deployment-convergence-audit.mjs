...
for(const token of ['Publish Consumer Standalone Installer','Build Kleenest App Family Android APKs','Kleenest-Consumer-Standalone-APK','Kleenest-Consumer.apk','mkdir -p apps/consumer-mobile/dist/legal','cp public/legal/*.html apps/consumer-mobile/dist/legal/','apps/consumer-mobile/dist/legal/privacy.html','apps/consumer-mobile/dist/legal/account-deletion.html','actions/configure-pages@v5','actions/upload-pages-artifact@v3','actions/deploy-pages@v4','path: apps/consumer-mobile/dist'])requireToken(installer,token,'Consumer installer deployment workflow');
...
if(!installer.includes('github.event.workflow_run.conclusion == \'success\''))throw new Error('Consumer installer deployment must require a successful family APK build.');

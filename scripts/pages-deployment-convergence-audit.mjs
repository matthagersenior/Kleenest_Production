...
for(const token of ["group: kleenest-app-family-android-${{ github.event_name == 'workflow_run' && github.event.workflow_run.head_branch == 'main' && 'main'", "cancel-in-progress: ${{ github.event_name != 'workflow_run' || github.event.workflow_run.conclusion == 'success' }}"])requireToken(familyAndroid,token,'Android family concurrency policy');

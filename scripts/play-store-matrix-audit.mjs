...
for (const [key, app] of Object.entries(matrix.apps)) {
  ...
  const easPath = `${root}/eas.json`;
  const eas = json(easPath);
  const production = eas.build?.production;

  must(
    config.includes(`package:'${app.package}'`) || config.includes(`package: '${app.package}'`),
    `${key}: Android package does not match compliance matrix`,
  );
  ...
  for (const legal of app.legal) {
    must(exists(`public/legal/${legal}.html`), `${key}: public ${legal} document missing`);
  }
  ...
  if (app.location.background) {
    ...
    const disclosure =
      read(`${root}/app/live-network.tsx`) +
      read(`${root}/app/signals.tsx`) +
  ...
  const appSource = exists(`${root}/app`)
    ? fs.readdirSync(`${root}/app`).filter((file) => file.endsWith('.tsx')).map((file) => read(`${root}/app/${file}`)).join('\n')
    : '';
  must(
    !/https?:\/\/(stripe\.com|paypal\.com)|openURL\([^)]*(checkout|subscribe|purchase)/i.test(appSource),
    `${key}: external digital checkout detected`,
  );
}
...
for (const token of ['report_user', 'report_review', 'block_user', 'unblock_user', 'accept_current_policies']) {
  must(consumerSource.includes(token), `consumer: UGC safety contract missing ${token}`);
}

const ownerModeration = read('apps/platform-mobile/app/moderation.tsx') + read('apps/platform-mobile/services/product.ts');
for (const token of ['admin_list_user_safety_reports', 'admin_list_ai_response_reports']) {
  must(ownerModeration.includes(token), `owner: moderation contract missing ${token}`);
}

const familySignup = read('apps/consumer-mobile/app/signup.tsx');
must(
  familySignup.includes("signup_intent:'family'") || familySignup.includes("signup_intent: 'family'"),
  'consumer: Family signup intent missing',
);
must(
  !/set.*subscription_tier.*family|update\([^)]*subscription_tier/i.test(familySignup),
  'consumer: Family signup must not self-entitle outside Play/backend authority',
);

const businessProduct = read('apps/business-mobile/services/product.ts');
for (const token of ['get_business_product_access', 'get_business_service_entitlement']) {
  must(businessProduct.includes(token), `business: server-authoritative Standard/Growth entitlement contract missing ${token}`);
}

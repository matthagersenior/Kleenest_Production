...
for(const token of ['Find a trusted bathroom.','BEST NEXT DECISION','What matters on this stop?','Start navigation','resolveConsumerSearchLocation','searchAreaOrigin','Searching near','Address, school, workplace, city or brand','listLocationTrustSummaries','listNearbyRestrooms',"router.push('/discover')",'<FreshnessHeatRing item={row}','<FreshnessHeatRing item={item} size={34}','<FreshnessHeatRing item={selected} size={34}'])if(!explore.includes(token))throw new Error(`Canonical Explore presentation missing contract: ${token}`);

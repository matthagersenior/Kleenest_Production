...
const home=requireAll('apps/consumer-mobile/app/index.tsx',['Find a bathroom you can trust.','FIND A BATHROOM','REVIEW','A recent visit','QR PROOF','QUICK ACTIONS','GAME CENTER','THE KLEENEST LOOP','YOUR PROGRESS','KLEENEST AI','TRUST GUIDE','ROUTE GUIDE','REVIEW DRAFT','COMMUNITY','OPEN COMMUNITY','Membership','Support']);
...
const quickReview=requireAll('apps/consumer-mobile/app/review/[id].tsx',['QUICK REVIEW','Dirty','Okay','Clean','Excellent','Submit review','consumer_quick_review']);
const location=requireAll('apps/consumer-mobile/app/location/[id].tsx',['KLEENEST RESTROOM','TrustStrip','BEFORE YOU GO','Start directions','Review this visit','LocationAmenityInventory','createMobileReview','uploadReviewPhotos','completeTrustMission',"import ReviewReportAction from '../../components/ReviewReportAction';",'<ReviewReportAction reviewId={String(item.id)}']);
...
const qr=requireAll('apps/consumer-mobile/app/qr.tsx',['QR is optional. Use it when a place has one.','Use whichever option is available.','Check in at this location','MANUAL FALLBACK','Check in without QR','CameraView','resolveQrAction','executeQrAction']);

for(const [name,source] of Object.entries({layout,home,explore,discover,progress,profile,prefs,play,social,quickReview,location,saved,route,activity:activitySource,notifications,membership,qr,ui})){

...
if(!failures.length){const read=file=>fs.readFileSync(file,'utf8'),trust=read(files[0]),missionStore=read(files[1]),explore=read(files[2]),saved=read(files[3]),route=read(files[4]),location=read(files[5]),locationPage=read(files[6]),play=read(files[7]);
 ...
 for(const forbidden of ['NEARBY TRUST MISSION','beginMission','startTrustMission(missionFromTrust','setSortMode(\'evidence\')','markerMission','markerBest','markerActive'])if(explore.includes(forbidden))failures.push(`Explore must keep advanced trust workflow out of the bathroom-finding path: ${forbidden}`);

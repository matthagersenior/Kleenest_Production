...
for(const token of ['getOfferReadiness','getPilotCapabilityDomains','updateOfferGovernance','updatePilotCapabilityDomain','getPilotSessions','createPilotSession','updatePilotSession'])
  if(!service.includes(token))failures.push(`KleenestOS pilot service missing ${token}`);

for(const token of ['Production ready','Pilot ready','Sample ready','Commercial state','Capability-level pilot controls','People & Access','Start a named pilot','Pilot sessions','Create Pilot Draft','Active pilots','Launch snapshot'])

...
if(!failures.length){
  ...
  if(amenityMigration.includes('review_amenity_feedback'))failures.push('Canonical amenity aggregation must not read legacy review feedback after evidence convergence.');
  ...
  for(const token of ["rpc('get_location_trust_quality'",'needs_reverification','contradiction_count','quality_score'])if(!trustService.includes(token))failures.push(`Mobile trust service missing ${token}.`);
  ...
  if(!workspaces.includes("client.rpc('business_restroom_trust_quality'"))failures.push('Business workspace service must load trust quality control-plane data.');
  for(const token of ['trustQuality','TRUST QUALITY CONTROL','Needs reverification','Evidence conflicts','contradiction_count','needs_reverification'])if(!business.includes(token))failures.push(`Business trust presentation missing ${token}.`);
}

...
const migrationPath='supabase/migrations/20260831073500_mobile_game_score_normalization_authority.sql';
const required=['apps/consumer-mobile/services/games.ts','apps/consumer-mobile/services/gameModes.ts','apps/consumer-mobile/services/gameScoring.ts','apps/consumer-mobile/app/games.tsx','apps/consumer-mobile/app/play.tsx','apps/consumer-mobile/app/_layout.tsx',migrationPath];
const failures=[];
for(const file of required)if(!fs.existsSync(file))failures.push(`missing mobile Game Center file: ${file}`);
if(!failures.length){
 const service=fs.readFileSync(required[0],'utf8'),modes=fs.readFileSync(required[1],'utf8'),scoring=fs.readFileSync(required[2],'utf8'),screen=fs.readFileSync(required[3],'utf8'),play=fs.readFileSync(required[4],'utf8'),layout=fs.readFileSync(required[5],'utf8'),migration=fs.readFileSync(migrationPath,'utf8');
 ...
 for(const code of codes)if(!modes.includes(`code:'${code}'`)||!migration.includes(`'${code}'`))failures.push(`Game Center missing canonical game authority: ${code}`);
 ...
 if(!modes.includes('strategyBudget:20')||!modes.includes('costs?:number[]')||!screen.includes('strategyTokens')||!screen.includes('Not enough evidence tokens'))failures.push('Stall Strategy must use a completable finite evidence budget.');

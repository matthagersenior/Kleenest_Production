...
if(!failures.length){
 ...
 for(const token of ['readNearbyContinuity','writeNearbyContinuity','preservedId','keyboardShouldPersistTaps','fallback.origin','cachedAgeLabel(cache.savedAt)'])if(!explore.includes(token))failures.push(`Explore continuity missing ${token}`);
 ...
 if(!/writeNearbyCache\(enriched,\s*\{[\s\S]*?selectedId:\s*preservedId,[\s\S]*?origin:\s*nextOrigin,[\s\S]*?radiusMeters:\s*(?:radius|result\.effectiveRadiusMeters)[\s\S]*?\}\)/.test(explore))failures.push('Live generic discovery must persist the last-good result context atomically, including the actual effective search radius.');
 if(!explore.includes("Live lookup failed. Showing cached nearby places")||!explore.includes("tap Search for a live result"))failures.push('Explore must clearly distinguish cached fallback from live discovery and expose explicit Search recovery.');

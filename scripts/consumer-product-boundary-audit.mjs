...
if(!failures.length){
 ...
 for(const token of ['Name or address is enough to start.','Add to Kleenest','Use GPS',"captureGPS('onsite_live')","?'map_pin':",'Save restroom knowledge','Take photo','Choose existing photo'])if(!discover.includes(token))failures.push(`Consumer contribution fast path missing capability: ${token}`);

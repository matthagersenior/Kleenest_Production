...
if(!failures.length){
 ...
 for(const token of ['Build Consumer Standalone Android APK','workflow_dispatch:','push:','NODE_ENV: production','npm run native:typecheck','native-consumer-recovery-audit.mjs','npx expo prebuild --platform android','./gradlew assembleRelease','actions/upload-artifact@v4','app-release.apk','Kleenest-Consumer-Standalone.apk',projectId])if(!androidStandalone.includes(token))failures.push(`Android standalone workflow missing ${token}`);

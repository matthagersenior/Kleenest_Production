...
if(!failures.length){
 ...
 if(eas?.build?.candidate?.android?.buildType!=='apk'||eas?.build?.candidate?.distribution!=='internal'||eas?.build?.candidate?.autoIncrement!==true)failures.push('Candidate Android profile must produce an auto-incremented internal release APK.');
 ...
 for(const profile of ['development','preview','candidate','production'])if(eas?.build?.[profile]?.env?.EAS_PROJECT_ID!==projectId)failures.push(`${profile} EAS profile must bind the canonical Expo project id.`);
 ...
 for(const script of ['start:dev-client','prebuild:android','eas:development:android','eas:preview:android','eas:candidate:android','eas:production:android'])if(!pkg.scripts?.[script])failures.push(`consumer mobile script missing ${script}`);
 ...
 for(const token of ['workflow_run:','workflows: ["Production CI"]','github.event.workflow_run.head_sha','npm run native:typecheck','native-consumer-recovery-audit.mjs','npx expo prebuild --platform android','./gradlew assembleRelease','actions/upload-artifact@v4','app-release.apk','SHA256SUMS.txt',projectId])if(!androidPreview.includes(token))failures.push(`Android candidate workflow missing ${token}`);
 if(!androidPreview.includes("github.event.workflow_run.conclusion == 'success'")||!androidPreview.includes("github.event.workflow_run.head_branch == 'main'"))failures.push('Android candidate artifacts must only auto-build from successful main Production CI runs.');

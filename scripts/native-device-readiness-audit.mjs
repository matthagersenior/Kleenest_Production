...
if(!failures.length){
 ...
 for(const token of ['workflow_run:','workflows: ["Production CI"]','github.event.workflow_run.head_sha','npm run native:typecheck','native-consumer-recovery-audit.mjs','npx expo prebuild --platform android','./gradlew assembleDebug','actions/upload-artifact@v4','app-debug.apk'])if(!androidPreview.includes(token))failures.push(`Android preview workflow missing ${token}`);

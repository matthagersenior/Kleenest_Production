...
const mobileLocationsPath='packages/mobile-core/src/locations.ts';
...
const failures=[];
for(const file of[componentPath,activityPath,socialPath,routePath,routeTrustPath,locationTrustPath,mobileCorePath,mobileLocationsPath,explorePath,savedPath,locationPath])if(!fs.existsSync(file))failures.push(`missing route/photo presentation file: ${file}`);
if(!failures.length){
 const component=fs.readFileSync(componentPath,'utf8'),activity=fs.readFileSync(activityPath,'utf8'),social=fs.readFileSync(socialPath,'utf8'),route=fs.readFileSync(routePath,'utf8'),routeTrust=fs.readFileSync(routeTrustPath,'utf8'),locationTrust=fs.readFileSync(locationTrustPath,'utf8'),mobileCore=fs.readFileSync(mobileCorePath,'utf8'),mobileLocations=fs.readFileSync(mobileLocationsPath,'utf8'),explore=fs.readFileSync(explorePath,'utf8'),saved=fs.readFileSync(savedPath,'utf8'),location=fs.readFileSync(locationPath,'utf8');
 ...
 const locationLoader=mobileLocations.match(/export async function getMobileLocations\([\s\S]*?\nexport async function getMobileLocation\(/)?.[0]||'';

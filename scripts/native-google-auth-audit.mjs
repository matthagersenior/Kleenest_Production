...
if(!relay.includes('getBrowserLocation')||!relay.includes("typeof location.search!=='string'")||(!relay.includes('!location||!callbackPresent(location)')&&!relay.includes('if(!location)return false;')))failures.push('Consumer-root OAuth relay must exit safely on native runtimes where window exists without browser location APIs.');

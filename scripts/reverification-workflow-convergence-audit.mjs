...
const files={
  ...
  lifecycle:'supabase/migrations/20260831173142_reverification_operations_lifecycle.sql',
...
if(!failures.length){
  ...
  const lifecycle=fs.readFileSync(files.lifecycle,'utf8').toLowerCase();
  ...
  for(const token of ['business_reverification_operations','business_manage_reverification_case','business_reverification_cases','consumer_reverification_cleared'])if(!lifecycle.includes(token))failures.push(`Reverification lifecycle migration missing ${token}.`);
  for(const token of ['revoke all on function public.business_reverification_queue(uuid) from public,anon','grant execute on function public.business_reverification_queue(uuid) to authenticated,service_role','revoke all on function public.start_reverification_trust_mission(uuid,text) from public,anon','grant execute on function public.complete_reverification_trust_mission(uuid) to authenticated,service_role'])if(!(workflow+mission+lifecycle).includes(token))failures.push(`Reverification authority missing ${token}.`);
  ...
  for(const token of ["rpc('business_reverification_operations'","rpc('business_create_reverification_qr'","rpc('business_manage_reverification_case'"])if(!businessService.includes(token))failures.push(`Business reverification service missing ${token}.`);
  for(const token of ['REVERIFICATION OPERATIONS','Run the restroom trust-quality queue','Launch reverification QR','priority_score','suggested_action','Assign to me','Resolved'])if(!businessPanel.includes(token))failures.push(`Business reverification lifecycle panel missing ${token}.`);

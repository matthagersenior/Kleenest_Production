...
const required=[
  ...
  'apps/consumer-mobile/services/contributionDraft.ts',
...
if(!failures.length){
  ...
  const contributionDraft=read(required[7]);
  ...
  if(eas?.build?.candidate?.android?.buildType!=='apk'||eas?.build?.candidate?.distribution!=='internal'||eas?.build?.candidate?.env?.KLEENEST_RELEASE_CHANNEL!=='candidate')failures.push('Candidate EAS profile must remain a production-like internal Android APK.');
  if(eas?.build?.production?.android?.buildType!=='app-bundle'||eas?.build?.production?.distribution!=='store'||eas?.build?.production?.autoIncrement!==true)failures.push('Production EAS profile must remain an auto-incremented Android app bundle.');
  for(const token of ["version: '1.0.0'","package: 'com.kleenest.app'","bundleIdentifier: 'com.kleenest.app'","ACCESS_FINE_LOCATION","CAMERA","expo-notifications",projectId])if(!config.includes(token))failures.push(`Native app config missing ${token}`);
  for(const token of ['Build Consumer Android Preview','workflow_run','Production CI','github.event.workflow_run.head_sha',`EAS_PROJECT_ID: ${projectId}`,'KLEENEST_RELEASE_CHANNEL: candidate','npm run native:typecheck','native-consumer-apk-convergence-audit.mjs','native-consumer-recovery-audit.mjs','native-device-readiness-audit.mjs','npx expo prebuild --platform android --non-interactive --clean','assembleRelease','app-release.apk','SHA256SUMS.txt','actions/upload-artifact'])if(!androidWorkflow.includes(token))failures.push(`Android artifact workflow missing ${token}`);

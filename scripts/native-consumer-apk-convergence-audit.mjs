...
if(!failures.length){
  ...
  for(const token of ['Build Consumer Standalone Android APK',`EAS_PROJECT_ID: ${projectId}`,'NODE_ENV: production','npm run native:typecheck','native-consumer-apk-convergence-audit.mjs','native-consumer-recovery-audit.mjs','native-device-readiness-audit.mjs','npx expo prebuild --platform android --clean','assembleRelease','app-release.apk','Kleenest-Consumer-Standalone.apk','actions/upload-artifact'])if(!androidWorkflow.includes(token))failures.push(`Standalone Android artifact workflow missing ${token}`);

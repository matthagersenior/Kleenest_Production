...
assert.match(explore, /Results are below/);
assert.match(explore, /resultsPeek/);
assert.match(signals, /expanded\?'−':'＋'/);
assert.doesNotMatch(signals, /expanded\?'Close ×':'Legend \+'/);
assert.match(explore, /useSafeAreaInsets/);
assert.match(explore, /searchPanelHeight/);
assert.match(explore, /onLayout=\{event=>setSearchPanelHeight/);
assert.match(explore, /mapChromeTop=searchPanelTop\+searchPanelHeight\+10/);

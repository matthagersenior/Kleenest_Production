...
if(!failures.length){
 ...
 if(!exploreCompact.includes('constexploreMapHeight=Math.max(320,Math.min(430,Math.round(windowHeight*0.38)))')||!exploreCompact.includes('style={[s.mapFrame,{height:exploreMapHeight}]}')||!exploreCompact.includes('mapFrame:{minHeight:380'))failures.push('Explore functional home must keep a usable bounded map while leaving result content discoverable in the first viewport.');

...
if(!failures.length){
 ...
 for(const token of ['Search this area','Results are below','resultsPeek','Address, school, workplace, city or brand','What matters on this stop?','Start directions','Add to route','BEST NEXT DECISION'])if(!explore.includes(token))failures.push(`Bathroom-first map-first Explore missing ${token}.`);
 ...
 if(!exploreCompact.includes('constexploreMapHeight=Math.max(380,Math.min(500,Math.round(windowHeight*0.56)))')||!exploreCompact.includes('style={[s.mapFrame,{height:exploreMapHeight}]}')||!exploreCompact.includes('mapFrame:{minHeight:380'))failures.push('Explore functional home must keep a usable adaptive map while leaving result content discoverable in the first viewport.');

...
if(!failures.length){
 ...
 for(const token of ['findLatestEligibleReviewCheckIn','setCheckInId(eligible?.id||null)','setComment(\'\')','setAmenityDraft({})','setReviewPhotos([])'])if(!location.includes(token))failures.push(`Location contribution recovery missing ${token}`);
 if(!/if\(submitting\|\|!checkInId(?:\|\|[^)]*)?\)return/.test(location))failures.push('Location contribution recovery must remain gated by an eligible check-in.');

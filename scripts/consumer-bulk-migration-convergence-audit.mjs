...
const failures=[];
const required=[
  'apps/consumer-mobile/app/_layout.tsx','apps/consumer-mobile/app/index.tsx','apps/consumer-mobile/app/explore.tsx','apps/consumer-mobile/app/location/[id].tsx','apps/consumer-mobile/app/profile.tsx','apps/consumer-mobile/app/preferences.tsx','apps/consumer-mobile/app/play.tsx','apps/consumer-mobile/app/social.tsx','apps/consumer-mobile/app/saved.tsx','apps/consumer-mobile/app/route.tsx','apps/consumer-mobile/app/qr.tsx','apps/consumer-mobile/app/activity.tsx','apps/consumer-mobile/app/notifications.tsx','apps/consumer-mobile/app/membership.tsx','apps/consumer-mobile/app/support.tsx','apps/consumer-mobile/app/account-deletion.tsx','packages/mobile-core/src/index.ts','apps/consumer-mobile/package.json','apps/consumer-mobile/app.config.ts'
];
for(const file of required)if(!fs.existsSync(file))failures.push(`missing consumer migration file: ${file}`);
if(!failures.length){
  const read=file=>fs.readFileSync(file,'utf8');
  const [layout,home,explore,location,profile,preferences,play,social,saved,route,qr,activity,notifications,membership,support,deletion,core,mobilePackage,appConfig]=required.map(read);
  for(const [name,title] of [['index','Home'],['explore','Explore'],['play','Play'],['social','Community'],['profile','Profile']]){
    if(!layout.includes(`name="${name}"`)||!layout.includes(`title:'${title}'`))failures.push(`primary consumer tab missing or renamed: ${title}`);
  }
  for(const hidden of ['games','route','qr','saved','activity','notifications','membership','preferences','support','account-deletion'])if(!new RegExp(`name=["']${hidden}["'][^>]*href:\\s*null`).test(layout))failures.push(`secondary consumer route must remain reachable but hidden from primary tabs: ${hidden}`);
  ...
  for(const token of ["router.push('/explore')",'/saved','/qr','THE KLEENEST LOOP','YOUR NETWORK'])if(!home.includes(token))failures.push(`Home missing rich bathroom-first activation capability: ${token}`);
  for(const token of ['listNearbyRestrooms','listAmenityCatalog','selectedAmenityNames',"pathname:'/route'",'captureConsumerRouteIntent','navigateUrl','listLocationTrustSummaries','readNearbyCache','writeNearbyCache'])if(!explore.includes(token))failures.push(`Explore missing mature discovery capability: ${token}`);
